#!/usr/bin/env python3
"""Round 36 live proof: capacity, catalog, FILES hist/handoff, SET persist."""
...
def main():
    ...
    marked_set1 = harvest(ser)
    click(q, dock_xy(0)[0], dock_xy(0)[1])
    set_first = wait_tok(ser, "SET READY", marked_set1, 4.0) or wait_tok(
        ser, "SET CSD", marked_set1, 2.5)
    ...
    files_multi_a = wait_tok(ser, "FILES READY", files1, 3.0) or wait_tok(
        ser, "FILES CSD", files1, 2.0)
    files2 = harvest(ser)
    click(q, dock_xy(1)[0], dock_xy(1)[1])
    files_multi_b = wait_tok(ser, "FILES READY", files2, 3.0) or wait_tok(
        ser, "FILES CSD", files2, 2.0)
    ...
    d15.shot(q, os.path.join(ART, "oscortex-round36-files-studio.png"))
    ...
    for i in (0, 2, 3, 4, 5):
        ...
        time.sleep(0.2)
    time.sleep(0.25)
    d15.shot(q, os.path.join(ART, "oscortex-round36-all-apps.png"))
    ...
    refuse_n = blob.count("FILE REFUSED FFFFFFFFFFFFFFF9")
    ow_refuse = sum(1 for line in blob.splitlines()
                    if "FILE REFUSED FFFFFFFFFFFFFFF9" in line
                    and "OPENWITH" in line)
    cap = {
        "wmMaxWindows": 20,
        "shmMax": 20,
        "procMax": 16,
    ...
    instance = {
        "set_first": set_first,
        "set_refocus": set_focus,
        "set_duplicate": set_dup,
        "files_multi_a": files_multi_a,
        "files_multi_b": files_multi_b,
        "policy": {
            "SET": "single-instance focus existing",
            "FILES": "multi-document always spawn",
            "STUDIO": "single-process multi-document",
            "BROWSE": "single-instance",
            "PLAY": "single-instance",
            "TAP": "single-instance",
        },
    }
    dock = {
        "pins_token": "DESK PINS" in blob,
        "pins_file": "PINS.DAT",
        "core_icons": ["SET", "FILES", "BROWSE", "PLAY", "STUDIO", "TAP"],
        "extras_from_catalog": True,
        "overflow_800": "icon_vis<=4",
        "overflow_1280": "icon_vis<=6 + chevron",
    }
    openwith = {
        "files_open_studio": handoff,
        "studio_openwith": studio_ow,
        "studio_open": studio_open,
        "refuse_fffffffff9": refuse_n,
        "openwith_refuse_lines": ow_refuse,
        "silent_notfound": ow_refuse == 0,
        "backoff_frames": 256,
        "protocol": "OPENWITH.DAT byte12 pending=1 consumed=0xFF",
    }
    write_json("oscortex-round36-capacity.json", cap)
    write_json("oscortex-round36-catalog.json", {
        "semantics": "FAT generation cache; scan idle/stale only",
        "excludes": ["DESK.ELF", "non-ELF", "invalid magic"],
        "n": catalog_n,
        "token": "WM CATALOG ",
        "cache_word": "wmPageWCatGen=fatWrites()",
        "refresh": "rescan when fatWrites moves or cache empty",
    })
    write_json("oscortex-round36-nav.json", nav)
    write_json("oscortex-round36-handoff.json", handoff)
    write_json("oscortex-round36-openwith.json", openwith)
    write_json("oscortex-round36-prefs.json", prefs)
    write_json("oscortex-round36-overlay.json", overlay)
    write_json("oscortex-round36-studio.json", studio)
    write_json("oscortex-round36-instance.json", instance)
    write_json("oscortex-round36-dock.json", dock)
    write_json("oscortex-round36-integrity.json", integrity)
    ...
    if tap_die:
        raise SystemExit("prove-round36: TAP DIE")
    if not catalog and catalog_n < 1:
        raise SystemExit("prove-round36: no catalog")
    return 0
