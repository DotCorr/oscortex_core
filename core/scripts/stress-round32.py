#!/usr/bin/env python3
"""Round 32 integrity: 5 min, no CPATH 3 after DESK READY, >=1000 frames."""
...
def main():
    ...
    dest = os.path.join(ART, "oscortex-round32-integrity.json")
    open(dest, "w").write(json.dumps(out, indent=2) + "\n")
    open(os.path.join(ART, "oscortex-round32-cpath.json"), "w").write(
        json.dumps({
            "round": 32,
            "source": "stress",
