#!/usr/bin/env python3
"""Round 39: 100 immediate first-frame captures after launcher show/hide.
...
bar_s = importlib.util.spec_from_file_location(
    "bar39", os.path.join(HERE, "capture-barrier-round39.py"))
bar39 = importlib.util.module_from_spec(bar_s)
bar_s.loader.exec_module(bar39)
...
def immediate_shot(q, ser, name):
    ...
    meta = bar39.shot_barrier(q, d15.shot, dest, ser)
    return dest, meta


def main():
    if len(sys.argv) < 3:
        raise SystemExit("usage: measure-round39.py <qmp> <serial>")
    ...
    for i in range(n):
        ...
        png, meta = immediate_shot(
            q, ser, "oscortex-round39-first-frame.png" if i == 0
            else "oscortex-round39-capture-show.png")
        ...
        hide_png, hide_meta = immediate_shot(
            q, ser, "oscortex-round39-capture-hide.png")
    ...
    dest = os.path.join(ART, "oscortex-round39-capture.json")
