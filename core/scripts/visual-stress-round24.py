#!/usr/bin/env python3
"""Round 24 visual stress: >=5 min AND >=100 menus AND >=100 focus.
...
def main():
    if len(sys.argv) < 4:
        raise SystemExit(
            "usage: visual-stress-round24.py <qmp> <serial> <outdir>")
            ...
            if geom is not None:
                ...
                try:
                    ...
                    if geom is not None and geom[2] >= 1000:
                        ...
                        try:
                            d15.press(q, ser, bx, by, "right",
                                      "FILES MENU", timeout=2)
                            menus += 1
                            take("loop-max-menu")
                        except Exception as e:
                            faults.append("files menu %s" % e)
    ...
    shots = {
        "menu": os.path.join(art, "oscortex-round24-fast-menu-focus.png"),
        "drag": os.path.join(art, "oscortex-round24-smooth-drag.png"),
    }
    ...
    open(os.path.join(art, "oscortex-round24-stress.json"), "w").write(
        json.dumps(payload, indent=2) + "\n")
    open(os.path.join(art, "oscortex-round24-frame-integrity.json"), "w").write(
        json.dumps(payload, indent=2) + "\n")
