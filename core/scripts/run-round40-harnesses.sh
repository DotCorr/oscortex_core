#!/usr/bin/env bash
# Round 40 isolated harness cluster. Does not touch the leftover QEMU.
set -u
CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ART="${ARTIFACTS_DIR:-/opt/cursor/artifacts}"
mkdir -p "$ART"
OUT="$ART/oscortex-round40-harnesses.json"
...
run_one vis-tokens "python3 '$CORE_DIR/scripts/test-vis-tokens.py'"
run_one task-pills "python3 '$CORE_DIR/scripts/test-task-pills.py'"
run_one app-matrix-src "python3 -c \"
import pathlib
p=pathlib.Path('$CORE_DIR/scripts/prove-round40.py').read_text()
need=['FILES','SET','BROWSE','PLAY','STUDIO','TAP','WM TASK A','ctrl_of','lifecycle100']
miss=[n for n in need if n not in p]
assert not miss, miss
print('app-matrix-src: PASS')
\""
