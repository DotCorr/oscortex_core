#!/usr/bin/env python3
"""Drive the Round 15 daily-drive QEMU: op-id pairing + cold restore.
...
COLD_RESTORE_P95_MS = float(os.environ.get("DRIVE_COLD_RESTORE_P95", "150"))
COLD_CYCLES = int(os.environ.get("DRIVE_COLD_CYCLES", "10"))
...
DEFN_RE = re.compile(r"^WM DEFN (ENQ|BEGIN|COMMIT) ([0-9A-F]+)")
IRQ_RE = re.compile(r"^WM IRQ ([0-9A-F]+)")
PREP_RE = re.compile(r"^WM PREP MAX")
IFHOLD_RE = re.compile(r"^WM IFHOLD ([0-9A-F]+) Y ([0-9A-F]+)")
LAT_RE = re.compile(
...
class Serial:
    ...
    _ARCHIVE = (
        ...
        "WM CSDHIT", "WM LIFE", "WM MOVE", "WM REAP", "WM MIN ",
        "WM FRAME", "WM FOCUS", "WM FPS", "WM PACE", "WM CHROME",
        "MOUSE ABS", "FB GOP", "VIEW MODE", "WM RAISE",
    ...
    def _ingest(self, text, into_seq=True):
        ...
        for ln in text.splitlines():
            ...
            if into_seq:
                ...
                dm = DEFN_RE.search(ln)
                if dm:
                    self.phase_events.append({
                        "token": "WM DEFN " + dm.group(1),
                        "opid": int(dm.group(2), 16),
                        "host_ms": t_ms,
                    })
                im = IRQ_RE.search(ln)
                if im:
                    self.phase_events.append({
                        "token": "WM IRQ",
                        "irq_ticks": int(im.group(1), 16),
                        "host_ms": t_ms,
                    })
                if PREP_RE.search(ln):
                    self.phase_events.append({
                        "token": "WM PREP MAX",
                        "host_ms": t_ms,
                    })
                hm = IFHOLD_RE.search(ln)
                if hm:
                    self.phase_events.append({
                        "token": "WM IFHOLD",
                        "reason": int(hm.group(1), 16),
                        "ticks": int(hm.group(2), 16),
                        "host_ms": t_ms,
                    })
...
def pair_inject(q, ser, events, timeout=2.5, want_opid=False, label=""):
    ...
    if phases:
        ...
        rec["defn_enq_ms"] = by.get("WM DEFN ENQ")
        rec["defn_begin_ms"] = by.get("WM DEFN BEGIN")
        rec["defn_commit_ms"] = by.get("WM DEFN COMMIT")
        rec["irq_ms"] = by.get("WM IRQ")
        rec["ifhold_ms"] = by.get("WM IFHOLD")
...
def parse_phases(text, serial_path=""):
    ...
    keys = (
        ...
        "FILES CFG", "WM DEFN ENQ", "WM DEFN BEGIN", "WM DEFN COMMIT",
        "WM IRQ", "WM PREP MAX", "OSGFX CHROME PREP",
        "SHM SHRINK", "SHM LIVE",
    )
    out = {}
    for k in keys:
...
def main():
    if len(sys.argv) < 4:
        raise SystemExit("usage: daily-drive-round15.py <qmp-port> <serial> <outdir>")
    ...
    if skip_boot:
        ...
        q.key("esc")
        time.sleep(0.2)
        shot(q, os.path.join(art, "oscortex-round15-tip-full-desktop.png"),
             os.path.join(fallback, "oscortex-round15-tip-full-desktop.png"))
        shot(q, os.path.join(outdir, "full-desktop.png"))
        try:
            copy_file(os.path.join(art, "oscortex-round15-tip-full-desktop.png"),
                      os.path.join(art, "oscortex-round15-deferred-wm.png"))
        except OSError:
            pass
        picture = picture_sentinels(
            os.path.join(art, "oscortex-round15-tip-full-desktop.png")
            if os.path.isfile(os.path.join(art, "oscortex-round15-tip-full-desktop.png"))
    ...
    try:
        copy_file(os.path.join(art, "oscortex-round15-cold-click.png"),
                  os.path.join(art, "oscortex-round15-cold-max.png"))
        copy_file(os.path.join(art, "oscortex-round15-cold-click.png"),
                  os.path.join(art, "oscortex-round15-atomic-max.png"))
        copy_file(os.path.join(art, "oscortex-round15-cold-click.png"),
                  os.path.join(art, "oscortex-round15-full-max.png"))
    except OSError:
        pass
    if PHASE_TIMELINES:
    ...
    walls["restore_cold"].append(timed_click(q, ser, FILES_MAX_MAXED_XY[0],
                                             ...
                                             label="restore_cold"))
    drain_idle(ser)
    shot(q, os.path.join(art, "oscortex-round15-cold-restore.png"),
         os.path.join(fallback, "oscortex-round15-cold-restore.png"))
    try:
        copy_file(os.path.join(art, "oscortex-round15-cold-restore.png"),
                  os.path.join(art, "oscortex-round15-first-restore.png"))
    except OSError:
        pass
    if PHASE_TIMELINES:
        PHASE_TIMELINES[-1]["qmp_fb_ms"] = round(
    ...
    atomic_png = (
        os.path.join(art, "oscortex-round15-atomic-max.png")
        if os.path.isfile(os.path.join(art, "oscortex-round15-atomic-max.png"))
        else os.path.join(art, "oscortex-round15-cold-max.png")
    )
    ...
    shot(q, os.path.join(art, "oscortex-round15-menu-survives.png"),
         os.path.join(fallback, "oscortex-round15-menu-survives.png"))
    try:
        copy_file(os.path.join(art, "oscortex-round15-menu-survives.png"),
                  os.path.join(art, "oscortex-round15-deferred-drag-menu.png"))
    except OSError:
        pass
    picture_sentinels(
        os.path.join(art, "oscortex-round15-menu-survives.png")
        if os.path.isfile(os.path.join(art, "oscortex-round15-menu-survives.png"))
        else os.path.join(outdir, "full-desktop.png"),
        SCREEN_W, SCREEN_H, expect_windows=True)
    ...
    extra_cold = max(0, COLD_CYCLES - 1)
    for _ci in range(extra_cold):
    ...
    shot(q, os.path.join(art, "oscortex-round15-no-hitch.png"),
         os.path.join(fallback, "oscortex-round15-no-hitch.png"))
    shot(q, os.path.join(outdir, "real-picture-latency.png"))
    picture_sentinels(
        os.path.join(art, "oscortex-round15-no-hitch.png")
        if os.path.isfile(os.path.join(art, "oscortex-round15-no-hitch.png"))
    ...
    metrics = {
        ...
        "deferred_wm": True,
        "deferred_menu_drag": True,
        "ifhold_n": text.count("WM IFHOLD "),
        "defn_enq_n": text.count("WM DEFN ENQ "),
        "defn_begin_n": text.count("WM DEFN BEGIN "),
        "irq_n": text.count("WM IRQ "),
        "cold_restore_p95_gate_ms": COLD_RESTORE_P95_MS,
        "pairing": "host_inject -> WM OPID -> WM DEFN ENQ/BEGIN/COMMIT -> WM PRES S <opid>",
        "max_restore_n": (
    ...
    try:
        open(os.path.join(art, "oscortex-round15-memory.json"), "w").write(mem_payload)
    except OSError:
    ...
    try:
        open(os.path.join(art, "oscortex-round15-shm.json"), "w").write(shm_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round15-shm.json"), "w").write(shm_payload)
    lat_path = os.path.join(art, "oscortex-round15-latency.json")
    try:
    ...
    try:
        open(os.path.join(art, "oscortex-round15-phase.json"), "w").write(phase_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round15-phase.json"), "w").write(phase_payload)
    ...
    rest_n = (wall_by.get("restore_cold") or {}).get("n", 0)
    rest_p95 = (wall_by.get("restore_cold") or {}).get("p95")
    if rest_n < COLD_CYCLES:
        raise SystemExit("cold restore cycles %s < %s" % (rest_n, COLD_CYCLES))
    if rest_p95 is not None and rest_p95 >= COLD_RESTORE_P95_MS:
        raise SystemExit("first restore p95 %s ms >= %s"
                         % (rest_p95, COLD_RESTORE_P95_MS))
    return 0
