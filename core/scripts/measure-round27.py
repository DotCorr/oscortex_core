#!/usr/bin/env python3
"""Round 27: achieved interactive present rate + p95 latency.

Measures pointer / drag / scroll / menu / max on the live leftover.
Reuses Round 24 pairing (OPID→PRES). Writes oscortex-round27-perf.json.
"""
...
def main():
    if len(sys.argv) < 3:
        raise SystemExit("usage: measure-round27.py <qmp> <serial>")
    ...
    payload = {
        ...
        "gates": {
            ...
            "pointer_fps": pointer["achieved_fps"] >= 4,
            "pointer_p95": (pointer["event_present_ms"]["p95"] or 999) < 400,
            "menu_n": menu["n"] >= 20,
            "menu_p95": (menu["event_present_ms"]["p95"] or 999) < 400,
        },
    }
    dest = os.path.join(art, "oscortex-round27-perf.json")
