#!/usr/bin/env bash
# Round 41 isolated harness cluster. Does not touch the leftover QEMU.
set -u
CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ART="${ARTIFACTS_DIR:-/opt/cursor/artifacts}"
mkdir -p "$ART"
OUT="$ART/oscortex-round41-harnesses.json"
...
import pathlib
p=pathlib.Path('$CORE_DIR/scripts/prove-round41.py').read_text()
need=['FILES','SET','BROWSE','PLAY','STUDIO','TAP','WM ACT I','ctrl_of','lifecycle100']
