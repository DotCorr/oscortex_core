#!/usr/bin/env bash
# Launch Round 32 leftover. Venus+scanout is BIOS -kernel + virtio-gpu-gl
...
RUN="${DRIVE_RUN:-$CORE_DIR/build/daily-drive-r32}"
mkdir -p "$RUN"
...
if [[ -z "$KERNEL_ELF" || ! -f "$KERNEL_ELF" ]]; then
  echo "launch-daily-drive-round32: promoting exact-tip core/build" >&2
  bash "$CORE_DIR/scripts/promote-core-build.sh"
  KERNEL_ELF="$CORE_DIR/build/kernel.elf"
  KERNEL_UEFI="$CORE_DIR/build/kernel-uefi.elf"
fi
[[ -f "$KERNEL_ELF" ]] || { echo "launch-daily-drive-round32: missing $KERNEL_ELF" >&2; exit 2; }
DISK="$RUN/disk.img"
ISO="$RUN/uefi.iso"
...
if [[ "$BIOS" != "1" ]]; then
  [[ -f "$KERNEL_UEFI" ]] || { echo "launch-daily-drive-round32: missing $KERNEL_UEFI" >&2; exit 2; }
  need_iso=0
...
if [[ -f "$RUN/qemu.pid" ]]; then
  ...
  if [[ -n "${old:-}" ]] && kill -0 "$old" 2>/dev/null; then
    if ps -p "$old" -o args= | grep -q 'oscortex-daily-drive-round32'; then
...
if [[ "$VENUS" == "1" ]]; then
  ...
  if ! "$QEMU_BIN" -device virtio-gpu-gl-pci,help 2>&1 | grep -q 'venus'; then
    echo "launch-daily-drive-round32: OSCORTEX_VENUS=1 but no venus= property" >&2
    exit 2
  fi
fi
...
nohup "$QEMU_BIN" \
  -name oscortex-daily-drive-round32 \
  "${BOOT_ARGS[@]}" \
  -m 512M -cpu qemu64 \
