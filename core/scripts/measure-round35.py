#!/usr/bin/env python3
"""Round 35: present-level overlay DONE pairing (kinds 7/8).
...
Writes oscortex-round35-perf.json (or OSCORTEX_PERF_OUT).
"""
...
def main():
    ...
    dest_name = os.environ.get("OSCORTEX_PERF_OUT", "oscortex-round35-perf.json")
    ...
    pair_name = os.environ.get(
        "OSCORTEX_PAIR_OUT", "oscortex-round35-pairing.json")
    if payload["path"] == "virtio":
        pair_name = os.environ.get(
            "OSCORTEX_PAIR_OUT", "oscortex-round35-virtio-pairing.json")
    pair = {
        "round": 32,
        "path": payload["path"],
        "token": payload["token"],
