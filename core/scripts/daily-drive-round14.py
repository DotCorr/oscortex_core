#!/usr/bin/env python3
"""Drive the Round 14 daily-drive QEMU: op-id pairing + cold restore.
...
class Serial:
    ...
    _ARCHIVE = (
        ...
        "FILES CFG", "WM DEFN", "WM IRQ", "WM PREP",
    )
...
def main():
    if len(sys.argv) < 4:
        raise SystemExit("usage: daily-drive-round14.py <qmp-port> <serial> <outdir>")
    ...
    if skip_boot:
        ...
        q.key("esc")
        time.sleep(0.2)
        shot(q, os.path.join(art, "oscortex-round14-tip-full-desktop.png"),
             os.path.join(fallback, "oscortex-round14-tip-full-desktop.png"))
        shot(q, os.path.join(outdir, "full-desktop.png"))
        try:
            copy_file(os.path.join(art, "oscortex-round14-tip-full-desktop.png"),
                      os.path.join(art, "oscortex-round14-deferred-wm.png"))
        except OSError:
            pass
        picture = picture_sentinels(
            os.path.join(art, "oscortex-round14-tip-full-desktop.png")
            if os.path.isfile(os.path.join(art, "oscortex-round14-tip-full-desktop.png"))
    ...
    shot(q, os.path.join(art, "oscortex-round14-cold-click.png"),
         os.path.join(fallback, "oscortex-round14-cold-click.png"))
    try:
        copy_file(os.path.join(art, "oscortex-round14-cold-click.png"),
                  os.path.join(art, "oscortex-round14-cold-max.png"))
        copy_file(os.path.join(art, "oscortex-round14-cold-click.png"),
                  os.path.join(art, "oscortex-round14-atomic-max.png"))
        copy_file(os.path.join(art, "oscortex-round14-cold-click.png"),
                  os.path.join(art, "oscortex-round14-full-max.png"))
    except OSError:
        pass
    if PHASE_TIMELINES:
    ...
    walls["restore_cold"].append(timed_click(q, ser, FILES_MAX_MAXED_XY[0],
                                             ...
                                             label="restore_cold"))
    drain_idle(ser)
    shot(q, os.path.join(art, "oscortex-round14-cold-restore.png"),
         os.path.join(fallback, "oscortex-round14-cold-restore.png"))
    if PHASE_TIMELINES:
        PHASE_TIMELINES[-1]["qmp_fb_ms"] = round(
            (time.perf_counter() - t_rest) * 1000.0, 1)
    atomic_png = (
        os.path.join(art, "oscortex-round14-atomic-max.png")
        if os.path.isfile(os.path.join(art, "oscortex-round14-atomic-max.png"))
        else os.path.join(art, "oscortex-round14-cold-max.png")
    )
    ...
    shot(q, os.path.join(art, "oscortex-round14-menu-survives.png"),
         os.path.join(fallback, "oscortex-round14-menu-survives.png"))
    picture_sentinels(
        os.path.join(art, "oscortex-round14-menu-survives.png")
        if os.path.isfile(os.path.join(art, "oscortex-round14-menu-survives.png"))
        else os.path.join(outdir, "full-desktop.png"),
        SCREEN_W, SCREEN_H, expect_windows=True)
    ...
    shot(q, os.path.join(art, "oscortex-round14-no-hitch.png"),
         os.path.join(fallback, "oscortex-round14-no-hitch.png"))
    shot(q, os.path.join(outdir, "real-picture-latency.png"))
    picture_sentinels(
        os.path.join(art, "oscortex-round14-no-hitch.png")
        if os.path.isfile(os.path.join(art, "oscortex-round14-no-hitch.png"))
    ...
    try:
        open(os.path.join(art, "oscortex-round14-memory.json"), "w").write(mem_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round14-memory.json"), "w").write(mem_payload)
    ...
    try:
        open(os.path.join(art, "oscortex-round14-shm.json"), "w").write(shm_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round14-shm.json"), "w").write(shm_payload)
    lat_path = os.path.join(art, "oscortex-round14-latency.json")
    try:
        open(lat_path, "w").write(payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round14-latency.json"), "w").write(payload)
    ...
    try:
        open(os.path.join(art, "oscortex-round14-phase.json"), "w").write(phase_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round14-phase.json"), "w").write(phase_payload)
