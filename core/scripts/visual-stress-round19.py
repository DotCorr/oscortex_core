#!/usr/bin/env python3
"""Round 19 visual stress: guest-FB dumps, SET reflow, inset focus, integrity."""
...
def main():
    if len(sys.argv) < 4:
        raise SystemExit(
            "usage: visual-stress-round19.py <qmp> <serial> <outdir>")
    ...
    shots = {
        "dirty": os.path.join(art, "oscortex-round19-dirty-regions.png"),
        "set": os.path.join(art, "oscortex-round19-set-responsive.png"),
        "focus": os.path.join(art, "oscortex-round19-rounded-focus.png"),
    }
    d15.shot(q, shots["dirty"], os.path.join(outdir, "dirty-regions.png"))
    d15.shot(q, shots["set"], os.path.join(outdir, "set-responsive.png"))
    d15.shot(q, shots["focus"], os.path.join(outdir, "rounded-focus.png"))
    ...
    open(os.path.join(art, "oscortex-round19-frame-integrity.json"), "w").write(
