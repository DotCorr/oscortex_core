#!/usr/bin/env python3
"""Round 36: present-level overlay DONE pairing (kinds 7/8).

Kinds: 1 pointer, 2 drag, 3 body/scroll, 4 menu open,
5 menu hide, 6 menu row, 7 launcher, 8 switcher.
Launcher/switcher wait exact opid+kind, not UART token wall.

Writes oscortex-round36-perf.json (or OSCORTEX_PERF_OUT).
"""
...
def overlay_kind_burst(q, ser, label, fire, dismiss, kind, n,
                       want_w=0, want_h=0, px=0):
    walls = []
    paired = []
    ...
    try:
        ...
        pass
    warm = walls[2:] if len(walls) > 2 else walls
    warm_s = sorted(warm)
    def pct(xs, p):
        if not xs:
            return 0.0
    ...
    return {
        "label": label,
        "n": len(walls),
        "pairing": "WM DONE opid+kind %d" % kind,
        "kind": kind,
        "p50_ms": pct(sorted(walls), 50),
        "p95_ms": pct(warm_s, 95),
        "max_ms": max(walls) if walls else 0,
        "warm_p95_ms": pct(warm_s, 95),
...
def write_overlay_json(art, launcher, switcher, park_ok):
    dest = os.path.join(art, os.environ.get(
        "OSCORTEX_OVERLAY_OUT", "oscortex-round36-overlay.json"))
...
def main():
    if len(sys.argv) < 3:
        raise SystemExit(
            "usage: measure-round36.py <qmp> <serial> [--overlay-only]")
    ...
    dest_name = os.environ.get("OSCORTEX_PERF_OUT", "oscortex-round36-perf.json")
    ...
    pair_name = os.environ.get(
        "OSCORTEX_PAIR_OUT", "oscortex-round36-pairing.json")
    if payload["path"] == "virtio":
        pair_name = os.environ.get(
            "OSCORTEX_PAIR_OUT", "oscortex-round36-virtio-pairing.json")
    pair = {
        "round": 32,
        "path": payload["path"],
        "token": payload["token"],
