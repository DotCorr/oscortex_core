#!/usr/bin/env python3
"""Round 41 integrity: 5 min, token_parse_error=0, no CPATH 3 after DESK READY."""
...
def main():
    ...
    for i in range(6):
        ...
        time.sleep(0.12)
    # 100 close/reopen cycles use live VIS + ctrl_of (prove-round41).
    import importlib.util as _ilu
    _ps = _ilu.spec_from_file_location("p41", os.path.join(HERE, "prove-round41.py"))
    p40 = _ilu.module_from_spec(_ps)
    p40_path = os.path.join(HERE, "prove-round41.py")
    ...
    try:
        ...
        out["token_err"] = str(e)
    dest = os.path.join(ART, "oscortex-round41-integrity.json")
    open(dest, "w").write(json.dumps(out, indent=2) + "\n")
    open(os.path.join(ART, "oscortex-round41-tokens.json"), "w").write(
        json.dumps({
            "round": 41,
            "token_parse_error": out.get("token_parse_error"),
            "token_rejected": out.get("token_rejected"),
            "uart": "IRQ enqueue + process drain; VIS line never spliced",
        }, indent=2) + "\n")
    open(os.path.join(ART, "oscortex-round41-cpath.json"), "w").write(
        json.dumps({
            "round": 41,
            "source": "stress",
