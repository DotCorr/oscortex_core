#!/usr/bin/env python3
"""Focused integrity/fault interval after a Round 31 kernel change."""
...
def main():
    ...
    out = {
        ...
        "oom": ("DESK READY" in blob) and (
            "OSGFX OOM" in blob.split("DESK READY", 1)[-1]),
        "qtimeout": "VIRTIO QTIMEOUT" in blob,
        "abort": "OSGFX ABORT" in blob,
        "tap_die": "TAP DIE " in blob,
        "cpath_compose": blob.count("WM CPATH 3"),
        "done_n": blob.count("WM DONE "),
        "frames": blob.count("WM FRAME N "),
    }
    dest = os.path.join(ART, "oscortex-round31-integrity.json")
