...
# Left of FILES (x=48) so a wallpaper card is not a SET hit.
WALL_XY = (16, min(220, SCREEN_H // 3))
...
def main():
    ...
    shot(q, os.path.join(art, "oscortex-round4-800x600.png" if SCREEN_W == 800
                         else "oscortex-round4-%dx%d.png" % (SCREEN_W, SCREEN_H)))
    ...
    shot(q, error_png)
    pair = os.path.join(art, "oscortex-round4-empty-error.png")
    ...
    for i in range(8):
        ...
        t0 = time.time()
        place(q, ser, x, y)
        got = wait_mark(ser, "WM FRAME", marked, 1.5)
        if got:
