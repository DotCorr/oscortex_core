#!/usr/bin/env python3
"""Guest-pixel Round 38 recording: QMP screendump sequence → ffmpeg.

Shows wallpaper-miss park, launcher (F4), switcher (Alt-Tab), dock apps,
FILES+STUDIO, and SET. Writes oscortex-daily-drive-round38-guest.mp4.
"""
...
def main():
    if len(sys.argv) < 5:
        raise SystemExit(
            "usage: guest-record-round38.py <qmp> <serial> <framedir> <mp4>")
    port = int(sys.argv[1])
    ...
    if str(serial_path).isdigit():
        ...
        serial_path = os.environ.get(
            "DRIVE_SERIAL_FILE",
            "/workspace/core/build/daily-drive-r38/serial.txt")
