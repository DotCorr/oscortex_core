#!/usr/bin/env python3
"""Round 37 integrity: 5 min, no CPATH 3 after DESK READY, >=1000 frames."""
...
def main():
    ...
    dest = os.path.join(ART, "oscortex-round37-integrity.json")
    open(dest, "w").write(json.dumps(out, indent=2) + "\n")
    open(os.path.join(ART, "oscortex-round37-cpath.json"), "w").write(
        json.dumps({
            "round": 37,
            "source": "stress",
