#!/usr/bin/env python3
"""Drive the Round 12 daily-drive QEMU: hitch gate + phase profile.
...
sock-only. Pairing: record last present seq, inject QMP, wait for sock
seq > last. That wall_ms is the deliverable. No >=1s samples; seq gaps
must be zero.
"""
...
def main():
    if len(sys.argv) < 4:
        raise SystemExit("usage: daily-drive-round12.py <qmp-port> <serial> <outdir>")
    ...
    if skip_boot:
        ...
        q.key("esc")
        time.sleep(0.2)
        shot(q, os.path.join(art, "oscortex-round12-tip-full-desktop.png"),
             os.path.join(fallback, "oscortex-round12-tip-full-desktop.png"))
        shot(q, os.path.join(outdir, "full-desktop.png"))
        picture = picture_sentinels(
            os.path.join(art, "oscortex-round12-tip-full-desktop.png")
            if os.path.isfile(os.path.join(art, "oscortex-round12-tip-full-desktop.png"))
    ...
    shot(q, os.path.join(art, "oscortex-round12-menu-survives.png"),
         os.path.join(fallback, "oscortex-round12-menu-survives.png"))
    picture_sentinels(
        os.path.join(art, "oscortex-round12-menu-survives.png")
        if os.path.isfile(os.path.join(art, "oscortex-round12-menu-survives.png"))
        else os.path.join(outdir, "full-desktop.png"),
        SCREEN_W, SCREEN_H, expect_windows=True)
    ...
    time.sleep(1.2)
    serial_fatal(serial_path, ser.read())
    shot(q, os.path.join(art, "oscortex-round12-full-max.png"),
         os.path.join(fallback, "oscortex-round12-full-max.png"))
    shot(q, os.path.join(art, "oscortex-round12-atomic-max.png"),
         os.path.join(fallback, "oscortex-round12-atomic-max.png"))
    atomic_png = (
        os.path.join(art, "oscortex-round12-atomic-max.png")
        if os.path.isfile(os.path.join(art, "oscortex-round12-atomic-max.png"))
        else os.path.join(art, "oscortex-round12-full-max.png")
    )
    ...
    shot(q, os.path.join(art, "oscortex-round12-no-hitch.png"),
         os.path.join(fallback, "oscortex-round12-no-hitch.png"))
    shot(q, os.path.join(outdir, "real-picture-latency.png"))
    picture_sentinels(
        os.path.join(art, "oscortex-round12-no-hitch.png")
        if os.path.isfile(os.path.join(art, "oscortex-round12-no-hitch.png"))
    ...
    try:
        open(os.path.join(art, "oscortex-round12-memory.json"), "w").write(mem_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round12-memory.json"), "w").write(mem_payload)
    ...
    try:
        open(os.path.join(art, "oscortex-round12-shm.json"), "w").write(shm_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round12-shm.json"), "w").write(shm_payload)
    lat_path = os.path.join(art, "oscortex-round12-latency.json")
    try:
        open(lat_path, "w").write(payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round12-latency.json"), "w").write(payload)
        print("WARN: latency JSON written to fallback")
    phase_payload = json.dumps(metrics["phase"], indent=2) + "\n"
    try:
        open(os.path.join(art, "oscortex-round12-phase.json"), "w").write(phase_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round12-phase.json"), "w").write(phase_payload)
    print(payload)
