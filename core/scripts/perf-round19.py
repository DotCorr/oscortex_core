#!/usr/bin/env python3
"""Round 19 interactive throughput: dirty pixels, fallbacks, event→present.

Does not conflate the 50 fps cap with achieved fps.
Writes /opt/cursor/artifacts/oscortex-round19-performance.json
"""
...
SCREEN_PX = 1280 * 720
# 320-wide SET tile, 2 appearance columns.
...
def parse_dmg(blob):
    ...
    full = []
    for ln in hits:
        m = re.match(r"WM DMG ([0-9A-F]+) RG ([0-9A-F]+) FL ([0-9A-F]+)", ln)
        if not m:
            continue
        px.append(int(m.group(1), 16))
        regs.append(int(m.group(2), 16))
        full.append(int(m.group(3), 16))
    return {"lines": hits[-8:], "px": px, "regs": regs, "full": full}


def parse_pace(blob):
    hits = [ln for ln in blob.splitlines() if ln.startswith("WM PACE ")]
    return hits[-4:]
...
def timed_ops(q, ser, label, points, btn=None):
    ...
    return {
        ...
        "event_present_ms": {
            ...
            "samples": [round(x, 1) for x in walls[:12]],
        },
        "guest_present_ms": {
            "n": len(presents),
...
def main():
    if len(sys.argv) < 3:
        raise SystemExit("usage: perf-round19.py <qmp> <serial-or-port>")
    port = int(sys.argv[1])
    ...
    drag = timed_ops(q, ser, "drag",
                     [(120 + i * 12, 55) for i in range(10)])
    d15.button(q, 120 + 108, 55, "left", False)

    focus_pts = [(120, 55) if i % 2 == 0 else (720, 55) for i in range(8)]
    ...
    payload = {
        ...
        "cap_fps": 50,
        "note": (
            "Achieved ops/sec and dirty px/frame, not the 50 fps cap. "
        ...
        "wm_pace_serial": parse_pace(blob),
        "wm_fps_serial": fps_txt[-400:],
    }
    path = os.path.join(art, "oscortex-round19-performance.json")
