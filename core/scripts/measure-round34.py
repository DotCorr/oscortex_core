#!/usr/bin/env python3
"""Round 34: strict WM DONE op+kind pairing (not first SCAN/FRAME).
...
Writes oscortex-round34-perf.json (or OSCORTEX_PERF_OUT).
"""
...
def main():
    if len(sys.argv) < 3:
        raise SystemExit("usage: measure-round34.py <qmp> <serial>")
    ...
    pair_name = os.environ.get(
        "OSCORTEX_PAIR_OUT", "oscortex-round34-pairing.json")
    if payload["path"] == "virtio":
        pair_name = os.environ.get(
            "OSCORTEX_PAIR_OUT", "oscortex-round34-virtio-pairing.json")
    pair = {
        "round": 32,
        "path": payload["path"],
        "token": payload["token"],
