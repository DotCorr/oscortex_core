#!/usr/bin/env python3
"""Guest-only Round 24 recording: cold launch + first drag + menu + max + SET.
...
def main():
    if len(sys.argv) < 5:
        raise SystemExit(
            "usage: guest-record-round24.py <qmp> <serial> <framedir> <mp4>")
    port = int(sys.argv[1])
    ...
    dump("max")
    geom = cs.live_files_xywh(serial_path, ser.archive or "") or geom
    # Formerly deadlocking: FILES menu on the maxed card, then restore.
    if geom[2] >= 1000:
    ...
    d15.press(q, ser, rx, ry, "left", "WM REQ", timeout=2)
    cs.wait_vis(ser, serial_path, n0=n3, pred=lambda g: g[2] < 1000, timeout=5)
    geom = cs.live_files_xywh(serial_path, ser.archive or "") or geom
    if geom[2] >= 1000:
        rx, ry = cs.ctrl_of(geom, "max")
        d15.press(q, ser, rx, ry, "left", "WM REQ", timeout=2)
