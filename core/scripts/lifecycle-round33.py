#!/usr/bin/env python3
"""Round 33: >=50 FILES close/reopen cycles. Fail on black body or CPATH 3.
...
def main():
    ...
    for i in range(N):
        ...
        time.sleep(0.15)
        geom = m31.files_geom(ser)
        shot = os.path.join("/tmp", "r33-life-%03d.png" % i)
        d15.shot(q, shot)
        ...
        if i == N - 1:
            d15.shot(q, os.path.join(ART, "oscortex-round33-reopen-clean.png"))
    ...
    dest = os.path.join(ART, "oscortex-round33-lifecycle.json")
    open(dest, "w").write(json.dumps(out, indent=2) + "\n")
    cpath_dest = os.path.join(ART, "oscortex-round33-cpath.json")
