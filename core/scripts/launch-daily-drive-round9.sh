#!/usr/bin/env bash
# Launch the Round 9 daily-drive QEMU: UEFI GOP 1280×720 + virtio-tablet.
# Leaves it running. Rebuilds the ISO when the kernel is newer, or when
# FORCE_ISO=1. Does not touch a live round-8 QEMU except by pid in this RUN dir.
set -euo pipefail
CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RUN="$CORE_DIR/build/daily-drive-r9"
mkdir -p "$RUN"
...
if [[ ! -f "$KERNEL_UEFI" ]]; then
  echo "launch-daily-drive-round9: missing $KERNEL_UEFI (build-kernel OSGFX_SKIA=1)" >&2
  exit 2
fi
if [[ ! -f "$DISK" ]]; then
...
if [[ "$need_iso" == "1" ]]; then
  if ! eval "$(bash "$CORE_DIR/scripts/find-limine.sh")"; then
    echo "launch-daily-drive-round9: limine not found" >&2
    exit 2
  fi
...
if [[ -f "$RUN/qemu.pid" ]]; then
  ...
  if [[ -n "${old:-}" ]] && kill -0 "$old" 2>/dev/null; then
    if ps -p "$old" -o args= | grep -q 'oscortex-daily-drive-round9'; then
...
nohup qemu-system-x86_64 \
  -name oscortex-daily-drive-round9 \
