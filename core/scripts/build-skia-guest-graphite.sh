...
if [[ ! -d "$SRC/.git" ]]; then
  echo "build-skia-guest-graphite: fetching Skia source" >&2
  SKIA_FETCH_ONLY=1 bash "$CORE/scripts/build-skia-graphite.sh"
fi
[[ -d "$SRC/.git" ]] || { echo "build-skia-guest-graphite: no Skia source" >&2; exit 1; }
[[ -x "$AR" ]] || { echo "build-skia-guest-graphite: need x86_64-elf-ar" >&2; exit 1; }
...
extra_cflags=[\"-DSK_CPU_LIMIT_SSE2\",\"-DSKCMS_FORCE_BASELINE\",\"-DSK_VULKAN\"]
"
...
[[ -f "$LIB" ]] || { echo "build-skia-guest-graphite: no $LIB" >&2; exit 1; }

SAMPLE=$(x86_64-elf-ar t "$LIB" | grep '\.o$' | head -1)
x86_64-elf-ar p "$LIB" "$SAMPLE" >"$CORE/build/skia-guest-graphite-sample.o"
if file "$CORE/build/skia-guest-graphite-sample.o" | grep -qi 'arm64'; then
  echo "build-skia-guest-graphite: archive is arm64 — refused" >&2
  exit 1
fi
if file "$CORE/build/skia-guest-graphite-sample.o" | grep -qi 'Mach-O'; then
  echo "build-skia-guest-graphite: archive is Mach-O — refused" >&2
  exit 1
fi
if ! file "$CORE/build/skia-guest-graphite-sample.o" | grep -qi 'ELF'; then
  echo "build-skia-guest-graphite: sample object is not ELF" >&2
  exit 1
fi

echo "skia-guest-graphite: $LIB"
