...
LAUNCH = os.path.join(HERE, "launch-daily-drive-round35.sh")
SIT = os.path.join(HERE, "boot-sit-in-round35.py")
...
def main():
    ...
    if old and old.isdigit():
        ...
        if "oscortex-daily-drive-round35" in args:
            os.kill(int(old), 15)
            time.sleep(0.6)
    ...
    dest = os.path.join(ART, "oscortex-round35-prefs.json")
    prev = {}
