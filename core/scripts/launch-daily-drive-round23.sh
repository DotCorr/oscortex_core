#!/usr/bin/env bash
# Launch the Round 23 daily-drive QEMU: UEFI GOP 1280×720 + virtio-tablet.
# Isolated KERNEL_UEFI only — never writes core/build/kernel*.elf.
set -euo pipefail
CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RUN="$CORE_DIR/build/daily-drive-r23"
mkdir -p "$RUN"
...
DISK="$RUN/disk.img"
ISO="$RUN/uefi.iso"
KERNEL_UEFI="${KERNEL_UEFI:-/tmp/oscortex-r23-kbuild/kernel-uefi.elf}"
if [[ ! -f "$KERNEL_UEFI" ]]; then
  echo "launch-daily-drive-round23: missing $KERNEL_UEFI" >&2
  exit 2
fi
if [[ ! -f "$DISK" ]] || [[ "${FORCE_DISK:-0}" == "1" ]]; then
  if [[ -f "$CORE_DIR/build/daily-drive-r22/disk.img" && "${FORCE_DISK:-0}" != "1" ]]; then
...
if [[ "$need_iso" == "1" ]]; then
  if ! eval "$(bash "$CORE_DIR/scripts/find-limine.sh")"; then
    echo "launch-daily-drive-round23: limine not found" >&2
    exit 2
  fi
...
if [[ -f "$RUN/qemu.pid" ]]; then
  ...
  if [[ -n "${old:-}" ]] && kill -0 "$old" 2>/dev/null; then
    if ps -p "$old" -o args= | grep -q 'oscortex-daily-drive-round23'; then
...
nohup qemu-system-x86_64 \
  -name oscortex-daily-drive-round23 \
