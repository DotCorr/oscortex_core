#!/usr/bin/env python3
"""Round 39 integrity: 5 min, no CPATH 3 after DESK READY, >=1000 frames."""
...
def main():
    ...
    try:
        ...
        out["token_err"] = str(e)
    dest = os.path.join(ART, "oscortex-round39-integrity.json")
    open(dest, "w").write(json.dumps(out, indent=2) + "\n")
    open(os.path.join(ART, "oscortex-round39-cpath.json"), "w").write(
        json.dumps({
            "round": 39,
            "source": "stress",
