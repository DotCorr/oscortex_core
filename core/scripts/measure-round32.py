#!/usr/bin/env python3
"""Round 32: strict WM DONE op+kind pairing (not first SCAN/FRAME).
...
Writes oscortex-round32-perf.json (or OSCORTEX_PERF_OUT).
"""
...
def main():
    if len(sys.argv) < 3:
        raise SystemExit("usage: measure-round32.py <qmp> <serial>")
    ...
    dest_name = os.environ.get("OSCORTEX_PERF_OUT", "oscortex-round32-perf.json")
    ...
    pair_name = os.environ.get(
        "OSCORTEX_PAIR_OUT", "oscortex-round32-pairing.json")
    if payload["path"] == "virtio":
        pair_name = os.environ.get(
            "OSCORTEX_PAIR_OUT", "oscortex-round32-virtio-pairing.json")
    pair = {
        "round": 32,
        "path": payload["path"],
        "token": payload["token"],
