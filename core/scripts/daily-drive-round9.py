#!/usr/bin/env python3
"""Drive the Round 9 daily-drive QEMU: full desktop + heap + wall-time.
...
seq > last. That wall_ms is the deliverable.
"""
...
def main():
    if len(sys.argv) < 4:
        raise SystemExit("usage: daily-drive-round9.py <qmp-port> <serial> <outdir>")
    ...
    shot(q, os.path.join(art, "oscortex-round9-real-picture-latency.png"),
         os.path.join(fallback, "oscortex-round9-real-picture-latency.png"))
    shot(q, os.path.join(outdir, "real-picture-latency.png"))
    picture_sentinels(
        os.path.join(art, "oscortex-round9-real-picture-latency.png")
        if os.path.isfile(os.path.join(art, "oscortex-round9-real-picture-latency.png"))
    ...
    try:
        open(os.path.join(art, "oscortex-round9-memory.json"), "w").write(mem_payload)
    except OSError:
        open(os.path.join(fallback, "oscortex-round9-memory.json"), "w").write(mem_payload)
    lat_path = os.path.join(art, "oscortex-round9-real-picture-latency.json")
    try:
