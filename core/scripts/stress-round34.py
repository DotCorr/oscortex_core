#!/usr/bin/env python3
"""Round 34 integrity: 5 min, no CPATH 3 after DESK READY, >=1000 frames."""
...
RUN = os.environ.get("DRIVE_RUN", "/workspace/core/build/daily-drive-r34c")
...
def main():
    ...
    dest = os.path.join(ART, "oscortex-round34-integrity.json")
    open(dest, "w").write(json.dumps(out, indent=2) + "\n")
    open(os.path.join(ART, "oscortex-round34-cpath.json"), "w").write(
        json.dumps({
            "round": 34,
            "source": "stress",
