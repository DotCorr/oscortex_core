#!/usr/bin/env python3
"""Guest-pixel Round 39 recording: QMP screendump sequence → ffmpeg.

Shows wallpaper-miss park, launcher (F4), switcher (Alt-Tab), dock apps,
FILES+STUDIO, and SET. Writes oscortex-daily-drive-round39-guest.mp4.
"""
...
def main():
    if len(sys.argv) < 5:
        raise SystemExit(
            "usage: guest-record-round39.py <qmp> <serial> <framedir> <mp4>")
    port = int(sys.argv[1])
