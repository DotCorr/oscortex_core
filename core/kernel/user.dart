...
void userSysExit(u64 frame) {
  ...
  // M20 (ADR-0034): THE EXIT CODE IS RECORDED BEFORE THE PROCESS BRANCH, not
  ...
  // M20 (ADR-0034): THIS LINE IS PRINTED ON BOTH PATHS NOW.
  //
...
void userSyscall(u64 frame) {
  ...
  /* Drain deferred WM ops in syscall context. IF stays clear except
   * inside drain/commit compose (wmIfHold*), which take wmMetaBusy
   * first. iretq restores the user RFLAGS. */
  if (wmActive() > u64(0)) {
    wmDefDrain();
  }
