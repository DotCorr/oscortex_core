#!/usr/bin/env bash
# core/tests/conformance/cef-plt/run.sh
#
# ADR-0169: bind official libcef memset@plt to OUR tiny LIBC.SO memset.
# PLAT.ELF dlopens LIBC then CEF; kernel patches the plant PLT stub;
# a derived call through that stub fills a buffer. Unbound PLT → #PF
# (GOT is past the RO+RX plant) and no LINE. ASK.ELF is BadArg.
#
# Not OnPaint. Not the rest of 1,336 UND / libdl.so.2 faces.
# Syscall 11 stays fdwait. Graphite / Venus fenced. TAP/FILES 64K/2MiB.
# Do not break cef-load / cef-wire / browse-paint floors.
#
# Usage: bash run.sh
...
fail() { echo "CEF-PLT: FAIL — $1" >&2; exit 1; }
setup_error() { echo "CEF-PLT: FAIL — $1" >&2; exit 2; }
...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-cef-plt.XXXXXX")" \
...
# Lean guest CRT (3 MiB) must keep the image under vmFineBytes (16 MiB).
...
ck; x86_64-elf-nm "$KERNEL_ELF" | grep -E '[[:space:]]T[[:space:]]+elfCefPlaceLibcMemset$' \
  || fail "kernel.elf has no elfCefPlaceLibcMemset — PLT bind door not linked"
ck; x86_64-elf-nm "$KERNEL_ELF" | grep -E '[[:space:]]T[[:space:]]+elfDlopenMapPlant$' \
  || fail "kernel.elf has no elfDlopenMapPlant"
ck; x86_64-elf-nm "$KERNEL_ELF" | grep -E '[[:space:]]T[[:space:]]+elfDlopenFindSym$' \
  || fail "kernel.elf has no elfDlopenFindSym"
...
ck; grep -q 'ADR-0169' "$CORE_DIR/docs/decisions/0169-memset-plt-binds-our-libc.md" \
  || fail "ADR-0169 file is missing"
ck; grep -q '0168 is full LOAD' "$CORE_DIR/docs/decisions/0169-memset-plt-binds-our-libc.md" \
  || fail "ADR-0169 stole 0168"
...
echo "STRUCTURAL: pass  PLT off pin; plant/floors held; Graphite fenced"
...
KEYS="$KEYS,$(typekeys 'proc spawn plat.elf'),ret,until:CEF PLT MEMSET ,until:LINE ,wait:800"
    ...
    def __init__(self, port):
        ...
        while time.time() < deadline:
            try:
                ...
                print("CEF-PLT: QEMU", hello.get("QMP", {}).get("version", {}))
                return
            except OSError as e:
                last = e
                time.sleep(0.2)
...
echo
echo "=== ASSERT ==="
python3 - "$SER" "$DERIVED" <<'PY' || fail "the boot does not satisfy the cef-plt door"
import re, sys
...
need(derived["plt_line"], "kernel PLT bind line")
need(derived["line"], "derived LINE from memset@plt fill")
...
# Anti-vacuity: PLT bind line must name a real platform VA, not zero.
...
print("    (PLAT.ELF: LIBC memset → CEF LOADs → memset@plt bound → LINE; ASK refused)")
PY

require_assertions "$ASSERTIONS_REQUIRED"
echo "CEF-PLT: PASS — official memset@plt bound to OUR libc memset; derived call through CEF PLT; leftover: rest of UND / libdl.so.2 / OnPaint"
exit 0
