#!/usr/bin/env python3
"""FAT16 volume for cef-dl (ADR-0174).

Full: PLAT.ELF + ASK.ELF + LIBDL.SO + SOMAP.TXT (libdl.so.2=LIBDL.SO).
Miss-map: same ELFs + LIBDL.SO, no SOMAP.TXT — cannot invent LINE.
Miss-so: SOMAP.TXT present, LIBDL.SO absent — mapping alone is not enough.
"""
...
def build(out, elf_path, libdl_path, somap_path):
    blob = read_blob(elf_path, "ELF", lo=64)
    blobs = {"PLAT.ELF": blob, "ASK.ELF": blob}
    names = ["PLAT.ELF", "ASK.ELF"]

    if libdl_path is not None:
        blobs["LIBDL.SO"] = read_blob(libdl_path, "LIBDL.SO", lo=64)
        names.append("LIBDL.SO")
    ...
    print("make-image: %s — PLAT/ASK %d, %s"
          % (os.path.basename(out), len(blob),
             ", ".join("%s %d" % (n, len(blobs[n])) for n in extras)))


def main():
    if len(sys.argv) < 3:
        raise SystemExit(
            "usage: make-image.py <out.img> <plat.elf> "
            "[--libdl path] [--somap path]")
    out = sys.argv[1]
    elf = sys.argv[2]
    libdl = None
    somap = None
    ...
    i = 0
    while i < len(args):
        if args[i] == "--libdl" and i + 1 < len(args):
            libdl = args[i + 1]
            i += 2
        elif args[i] == "--somap" and i + 1 < len(args):
            somap = args[i + 1]
            i += 2
        else:
            raise SystemExit("make-image: unknown arg %r" % args[i])
    build(out, elf, libdl, somap)
