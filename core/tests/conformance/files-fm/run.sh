...
ck; grep -q 'const int fileStoreBytes = 3584;' "$FILE_SRC" \
  || fail "fileStoreBytes is no longer 2560"
FILE_STORE=$(x86_64-elf-readelf -sW "$BUILD_DIR/kmain.o" \
  | awk '$8=="fileStore"{print $3+0; exit}')
ck; [[ "$FILE_STORE" -eq 3584 ]] || fail "fileStore is ${FILE_STORE:-missing} bytes, expected 2560"
...
ck; grep -q 'list_sel' "$FILES_C" \
  || fail "files.c has no list_sel"
ck; grep -q 'FILES SEL' "$FILES_C" \
  || fail "files.c does not print FILES SEL"
ck; grep -q 'FILES BACK' "$FILES_C" \
