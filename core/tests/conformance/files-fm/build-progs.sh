...
# THE ROWS ARE OUTLINES, NOT CELLS (ADR-0192). `osxui_label_fb` is the 8x16
# bitmap-cell path in osgfx_glyph.c; a FILES that still called it would still
# be stamping paper letters into its own shm. The replacement is
# `osxui_app_label_box` -> WM_OP_PAINT -> osgfx_text.
# Comments may (and do) name the cell path they replaced; code may not.
grep -vE '^[[:space:]]*(\*|/\*|//)' "$SRC" | grep -q 'osxui_label_fb' \
  && fail "files.c still calls osxui_label_fb — the rows are still 8x16 cells"
grep -q 'osxui_app_label_box' "$SRC" \
  || fail "files.c does not reach the osxui app text API"
grep -q 'WM_TEXT_LABEL_PX' "$SRC" \
  || fail "files.c does not name a pixel size — outlines are size-parametric"
if [[ "$NO_ICON" == "0" ]]; then
  grep -q 'osxui_icon_fb' "$SRC" \
    || fail "files.c does not call osxui_icon_fb"
  x86_64-elf-nm --undefined-only "$OUT/files.o" | grep -q 'osxui_label_fb' \
    && fail "files.o still binds osxui_label_fb"
