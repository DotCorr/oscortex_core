#!/usr/bin/env python3
"""FAT16 volume for plat-rel.

Full image: PLAT.ELF + ASK.ELF (identical DYNAMIC bytes) + LD.SO.
Nodyn image: PLAT.ELF (INTERP only) + LD.SO — works as today.
"""
...
def main():
    if len(sys.argv) != 4:
        raise SystemExit("usage: make-image.py <out.img> <plat.elf> <ld.so>")
    out, elf_path, ld_path = sys.argv[1], sys.argv[2], sys.argv[3]
    ...
    ld = open(ld_path, "rb").read()
    if len(ld) < 64:
        raise SystemExit("make-image: ld.so is too small")
    if len(ld) > 65536:
        raise SystemExit("make-image: ld.so exceeds the 64 KiB cap")

    blobs = {"PLAT.ELF": blob, "ASK.ELF": blob, "LD.SO": ld}
    names = ["PLAT.ELF", "ASK.ELF", "LD.SO"]
    ...
    print("make-image: %s — PLAT.ELF/ASK.ELF %d bytes + LD.SO %d bytes"
          % (os.path.basename(out), len(blob), len(ld)))
