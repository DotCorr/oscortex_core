#!/usr/bin/env python3
"""FAT16 volume with optional CLIP.MP4 and PLAY.ELF. Geometry matches apps1."""
import os
import struct
import sys
...
def main():
    if len(sys.argv) < 2:
        raise SystemExit("usage: make-image.py <out.img> [CLIP.MP4|-] [PLAY.ELF]")
    out = sys.argv[1]
    blobs = {"KEEP.TXT": b"keep\n"}
    if len(sys.argv) >= 3 and sys.argv[2] not in ("", "-"):
        clip = open(sys.argv[2], "rb").read()
        if len(clip) < 32:
            raise SystemExit("make-image: clip too small")
        if clip[:8] == b"OSCXPRG1":
            raise SystemExit("make-image: clip is OSCXPRG1, not H.264")
        blobs["CLIP.MP4"] = clip
    if len(sys.argv) >= 4 and sys.argv[3] not in ("", "-"):
        elf = open(sys.argv[3], "rb").read()
        if elf[:8] != b"OSCXPRG1" and elf[:4] != b"\x7fELF":
            raise SystemExit("make-image: PLAY.ELF is not ELF")
        blobs["PLAY.ELF"] = elf
    ...
    for name in blobs:
        ...
        while len(chain) < need:
            if c >= CLUSTER_COUNT + 2:
                raise SystemExit("make-image: out of clusters")
            if c not in taken:
                chain.append(c)
                taken.add(c)
            c += 1
    ...
    with open(out, "wb") as f:
        f.write(bytes(img))
    print("make-image: %s — %s" % (os.path.basename(out),
                                   ", ".join("%s %d" % (n, len(blobs[n]))
                                             for n in blobs)))
