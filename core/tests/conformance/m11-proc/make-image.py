#!/usr/bin/env python3
"""core/tests/conformance/m11-proc/make-image.py
...
told TWO sector numbers instead of one.
    ...
    make-image.py <out.img> <progA.elf> <progB.elf> [--json]
...
SLOTS = {"A": 0x20, "B": 0xA0, "Bhold": 0x120}
...
def mutate_hold(blob):
    """progB with `jmp .` (EB FE) written over the first instruction at e_entry.

    THE THIRD PROGRAM ON THIS DISK IS PROCESS B THAT NEVER FINISHES, and it
    exists for one reason: the page tables have to be read out of guest memory
    while BOTH address spaces are live, and both address spaces stop being live
    the moment either process exits or faults.

    m10-elf/make-image.py's `spin` did the same thing for the same reason. Two
    bytes, at e_entry, so nothing else about the program changes: the same
    segments, the same p_flags, the same number of pages, the same everything
    the isolation check compares. A program built separately to hold would have
    been a third variable in a check about two.
    """
    b = bytearray(blob)
    entry = int.from_bytes(blob[24:32], "little")
    off = file_offset(blob, entry)
...
def build(paths):
    ...
    for name, hdr_lba in sorted(SLOTS.items()):
        blob = open(paths["B"], "rb").read() if name == "Bhold" \
            else open(paths[name], "rb").read()
        if name == "Bhold":
            blob = mutate_hold(blob)
        image_lba = hdr_lba + 1
...
def verify(path, paths, layout):
    ...
    for name, info in layout.items():
        want = mutate_hold(open(paths["B"], "rb").read()) if name == "Bhold" \
            else open(paths[name], "rb").read()
    ...
    a = blob[layout["A"]["image_lba"] * SECTOR:
             layout["A"]["image_lba"] * SECTOR + layout["A"]["bytes"]]
    b = blob[layout["B"]["image_lba"] * SECTOR:
             layout["B"]["image_lba"] * SECTOR + layout["B"]["bytes"]]
    if a == b:
    ...
    # Bhold must differ from progB in EXACTLY two bytes, or it is not "the same
    # program that does not finish" and the isolation check is comparing two
    # things that differ in more than one way.
    pb = open(paths["B"], "rb").read()
    ph = mutate_hold(pb)
    diff = sum(1 for x, y in zip(pb, ph) if x != y)
    if diff != 2:
        fails.append("Bhold differs from progB.elf in %d bytes, expected exactly 2"
                     % diff)
...
def main():
    if len(sys.argv) < 4:
        ...
        sys.exit(2)
    out = sys.argv[1]
    paths = {"A": sys.argv[2], "B": sys.argv[3]}
    ...
    if "--emit" in sys.argv:
        ...
        open(os.path.join(d, "prog-Bhold.elf"), "wb").write(
            mutate_hold(open(paths["B"], "rb").read()))
