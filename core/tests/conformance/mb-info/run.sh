...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-mb-info.XXXXXX")" || setup_error "could not create a temp workdir"
...
capture_log "$WORKDIR/qemu.log" QEMU_STATUS -- timeout 5 qemu-system-x86_64 -kernel "$KERNEL_ELF" -m 128M -serial "file:$SERIAL_CAPTURE" -display none -no-reboot
...
ck; if ! cmp -s "$PREFIX" "$EXPECTED"; then
