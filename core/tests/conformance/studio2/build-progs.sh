#!/usr/bin/env bash
# core/tests/conformance/studio2/build-progs.sh
#
# STUDIO.ELF from studio.c (osframe.h) plus APP1.ELF from apps1/prog.c
# so a catalog launch has a resident child to start.
...
grep -q 'SYS_KBDEVENT' "$SRC" \
  || fail "studio.c never pops kbdevent — the derived key has no consumer"
...
x86_64-elf-readelf -lW "$OUT/app1.elf" | awk '$1=="LOAD"' | grep -q "RWE" \
  && fail "app1.elf has a W+X segment"
...
echo "build-progs: PASS — $OUT/studio.elf ($BYTES bytes) and $OUT/app1.elf ($ABYTES bytes)"
exit 0
