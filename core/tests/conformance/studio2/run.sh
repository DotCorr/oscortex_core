#!/usr/bin/env bash
# core/tests/conformance/studio2/run.sh
#
# STUDIO2 launch — from STUDIO.ELF, a derived key starts a catalog name
# as a resident process. docs/design/osxstudio.md. Listing is STUDIO1;
# this is launch. Not an IDE, not live-edit, not a guest Dart SDK.
#
# STUDIO.ELF reads APPS.TXT. Key '1' (or a click on the first hit strip)
# calls spawn(26) on the first planted name. Serial shows APP1's HEAP
# line and studio still listed. Negative: no key → APP1 does not start.
# Syscall 26, no oslibc.h name, no help, no last .bss theft.
#
# Usage: bash run.sh
...
fail() { echo "STUDIO2: FAIL — $1" >&2; exit 1; }
setup_error() { echo "STUDIO2: FAIL — $1" >&2; exit 2; }
...
# Launch does not call osgfx Skia or osmedia. Same switches as studio2b.
...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-studio2.XXXXXX")" \
...
ck; grep -q 'procSysSpawnNo' "$CORE_DIR/kernel/proc.dart" \
  || fail "proc.dart has no procSysSpawnNo"
ck; grep -q 'const int procSysSpawnNo = 26;' "$CORE_DIR/kernel/proc.dart" \
  || fail "procSysSpawnNo is not 26"
ck; grep -q 'procSysSpawn' "$CORE_DIR/kernel/user.dart" \
  || fail "userSyscall does not dispatch spawn"
...
ck; ! grep -n 'procSysSpawn' -A80 "$CORE_DIR/kernel/proc.dart" \
  | grep -q '@bss' \
  || fail "spawn donated .bss"
ck; ! grep -qiE 'studio\.c|STUDIO\.ELF|STUDIO1|STUDIO2|osxstudio' "$CORE_DIR/kernel/"*.dart \
  || fail "a kernel .dart names STUDIO — spawn is generic, not a studio hook"
ck; ! grep -E 'nic\.dart|usb\.dart|ahci\.dart' "$CORE_DIR/kernel/proc.dart" \
  || fail "proc.dart grew a nic/usb/ahci dependency"
...
echo "$REG_OUT"
echo "STRUCTURAL: pass  spawn 26, osframe.h, help 2511, last .bss wmeventStore"
...
echo "DERIVED: $(d token_0) key=$(d key_qcode) hit=$(d hit_x),$(d hit_y)"
    ...
    def __init__(self, port):
        ...
        while time.time() < deadline:
            try:
                ...
                print("STUDIO2: QEMU", hello.get("QMP", {}).get("version", {}))
                return
            except OSError as e:
                last = e
                time.sleep(0.2)
...
echo
echo "=== BOOT A — spawn STUDIO.ELF, derived key launches APP1 ==="
# Click focuses the strip (D9) so the digit reaches kbdevent, then '1'.
...
drive_session "$WORKDIR/main" "$KEYS" "main"
SERIAL="$WORKDIR/main/serial.txt"

echo
echo "=== BOOT B — no key, APP1 must not start ==="
...
ck; grep -qE '^(M1 END|>)' "$SERIAL" \
  || fail "the prompt never returned after studio spawn"
...
ck; grep -qF -- "$(d app1_hello)" "$NSER" \
  && fail "the negative boot printed APP1 hello without a key"
...
echo "CHECK: pass  APP1 launched from catalog key; no-key boot stayed listed"

require_assertions "$ASSERTIONS_REQUIRED"
echo "STUDIO2: PASS — STUDIO.ELF listed APPS.TXT, derived key launched APP1.ELF as a resident process (HEAP 1); studio stayed listed; no key did not start APP1; spawn is syscall 26; help 2511; last .bss wmeventStore. Listing is STUDIO1; this is launch. Not live-edit and not a Dart SDK."
exit 0
