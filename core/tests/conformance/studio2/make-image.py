#!/usr/bin/env python3
"""core/tests/conformance/studio2/make-image.py

FAT16 volume for STUDIO2 launch: STUDIO.ELF, APP1.ELF, and a planted
catalog APPS.TXT that lists APP1. Geometry matches studio1 / apps1.

    make-image.py <out.img> <studio.elf> <app1.elf> [--json]
"""
...
def main():
    ...
    if len(args) != 3:
        raise SystemExit("usage: make-image.py <out.img> <studio.elf> <app1.elf>")
    out, studio, app1 = args

    catalog = CATALOG
    if b"APP1.ELF" not in catalog:
        raise SystemExit("make-image: catalog does not list APP1.ELF")
    blobs = {
    ...
    chains = {}
    c = 2
    for name in ("STUDIO.ELF", "APP1.ELF", "APPS.TXT"):
    ...
    if want_json:
        ...
        print("make-image: %s — STUDIO.ELF %d, APP1.ELF %d, APPS.TXT %d"
              % (os.path.basename(out), len(blobs["STUDIO.ELF"]),
                 len(blobs["APP1.ELF"]), len(blobs["APPS.TXT"])))
