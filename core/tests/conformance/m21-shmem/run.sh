...
ck; [[ $(( 16#$WM_OFF + WM_SIZE )) -eq "$DART_BSS" ]] \
  || fail "wmStore ends at $(( 16#$WM_OFF + WM_SIZE )) and kmain.o's .bss is $DART_BSS — D4's block is not last"
ASM_BSS_HEX=$(x86_64-elf-objdump -h "$CORE_DIR/build/kdata.o" | awk '$2==".bss"{print $3; exit}')
ck; [[ $(( DART_BSS + 16#$ASM_BSS_HEX )) -eq 22336 ]] \
  || fail "the kernel's mutable static storage is $(( DART_BSS + 16#$ASM_BSS_HEX )) bytes, expected 22336 — 22016 through the D1/M21 merge, plus D4's wmStore 320 (ADR-0050, the compositor's window table). If that changed, it changed deliberately and GAP-0053's running total and every harness that subtracts a later block move with it."
