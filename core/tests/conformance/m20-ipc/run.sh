...
[[ "$KDATA_BSS" -eq 21856 ]] || fail "the kernel's mutable static storage is $KDATA_BSS bytes, expected 21856 — 14368 through M19, plus chanStore's 2624, plus S0's ioctlStore 512 (ADR-0033), plus M21's shmStore 4352 (ADR-0035). If that changed, it changed deliberately and this number, GAP-0053's running total, and every harness that subtracts a later block move with it."

# M21 (ADR-0035) added a block AFTER S0's and is the last one in .bss now, so it
# is subtracted FIRST -- exactly the accounting S0 itself gave M20.
SHM_STORE_SIZE=$(bsssize shmStore)
[[ "$SHM_STORE_SIZE" == "4352" ]] || fail "shmStore is ${SHM_STORE_SIZE:-missing} bytes, expected 4352 (ADR-0035)"
