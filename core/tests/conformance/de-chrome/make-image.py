#!/usr/bin/env python3
"""core/tests/conformance/de-chrome/make-image.py

FAT16 volume with two 8.3 ELFs: WIN.ELF then PING.ELF. Directory
order is the launch-row order `wm de` caches. Geometry matches apps1
so fsck_msdos accepts it.

    make-image.py <out.img> <win.elf> <ping.elf>
"""
...
def main():
    if len(sys.argv) != 4:
        raise SystemExit("usage: make-image.py <out.img> <win.elf> <ping.elf>")
    out, elf1, elf2 = sys.argv[1], sys.argv[2], sys.argv[3]

    blobs = {
        "WIN.ELF": open(elf1, "rb").read(),
        "PING.ELF": open(elf2, "rb").read(),
    }
    if blobs["WIN.ELF"] == blobs["PING.ELF"]:
        raise SystemExit("make-image: the two ELFs are byte-identical")

    taken = set()
    chains = {}
    c = 2
    for name in ("WIN.ELF", "PING.ELF"):
    ...
    add("WIN.ELF", 0x20, chains["WIN.ELF"][0], len(blobs["WIN.ELF"]))
    add("PING.ELF", 0x20, chains["PING.ELF"][0], len(blobs["PING.ELF"]))
    ...
    print("make-image: %s — WIN.ELF %d, PING.ELF %d"
          % (os.path.basename(out), len(blobs["WIN.ELF"]),
             len(blobs["PING.ELF"])))
