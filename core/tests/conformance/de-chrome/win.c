/* core/tests/conformance/de-chrome/win.c
 *
 * Resident compositor client for the DE-chrome harness. Same shape as
 * d3-session SIDE=0: attach, paint, commit, spin+yield. Geometry is
 * named so derive.py reads it. Close / min / body clicks land on this
 * window; PING.ELF is the spawn target.
 */
...
static volatile u64 marker = 0x00DE000100DE0001UL;
static u64 scratch[8];

static const char msg_attach[] = "DE WIN ATTACH\n";
static const char msg_commit[] = "DE WIN COMMIT\n";
...
void _start(void) {
  ...
  if (frames >= WM_FLOOR) {
    die(0xDE000004UL | (frames << 32));
  }
  wr(msg_commit, sizeof(msg_commit) - 1);
  scratch[0] = frames;
  if (marker != 0x00DE000100DE0001UL) {
    die(0xDE000006UL);
  }
