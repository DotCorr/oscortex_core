#!/usr/bin/env bash
# ONE freestanding ELF (two FAT names) + one tiny ET_DYN.
...
clang "${SO_CFLAGS[@]}" "$SCRIPT_DIR/so.c" -o "$OUT/so.o" \
  || fail "clang could not compile so.c"
x86_64-elf-ld -shared -z max-page-size=0x1000 --build-id=none \
  -o "$OUT/tiny.so" "$OUT/so.o" \
  || fail "x86_64-elf-ld could not link tiny.so"
[[ -s "$OUT/tiny.so" ]] || fail "no tiny.so"
x86_64-elf-readelf -hW "$OUT/tiny.so" | grep -q "DYN (Shared object" \
  || fail "tiny.so is not ET_DYN"
x86_64-elf-nm "$OUT/tiny.so" | grep -q ' D so_mark$' \
  || fail "tiny.so has no exported so_mark"
x86_64-elf-readelf -dW "$OUT/tiny.so" | grep -q '(HASH)' \
  || fail "tiny.so has no DT_HASH"
SO_BYTES=$(wc -c <"$OUT/tiny.so" | tr -d ' ')
[[ "$SO_BYTES" -le 65536 ]] || fail "tiny.so is $SO_BYTES bytes — image cap is 64 KiB"

echo "build-progs: PASS — $OUT/plat.elf ($BYTES bytes) + tiny.so ($SO_BYTES bytes)"
exit 0
