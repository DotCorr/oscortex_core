#!/usr/bin/env python3
"""FAT16 volume: identical PLAT.ELF / ASK.ELF plus TINY.SO.

Only the 8.3 name may honour dlopen (ADR-0144). TINY.SO is the
mapped ET_DYN. MISS.SO is deliberately absent.
"""
...
def main():
    if len(sys.argv) != 4:
        raise SystemExit("usage: make-image.py <out.img> <plat.elf> <tiny.so>")
    ...
    blobs = {"PLAT.ELF": blob, "ASK.ELF": blob, "TINY.SO": so}
    ...
    chains = {}
    c = 2
    for name in ("PLAT.ELF", "ASK.ELF", "TINY.SO"):
    ...
    add("TINY.SO", 0x20, chains["TINY.SO"][0], len(blobs["TINY.SO"]))
    ...
    print("make-image: %s — PLAT/ASK %d bytes, TINY.SO %d (MISS.SO absent)"
