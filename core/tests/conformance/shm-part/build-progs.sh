#!/usr/bin/env bash
# core/tests/conformance/shm-part/build-progs.sh
#
# Freestanding PROG for ADR-0160 (no oslibc — shmmap range unnamed).
...
grep -q 'PART_OFF' "$SCRIPT_DIR/prog.c" || fail "prog.c never names PART_OFF"
grep -q 'SHM_BADLEN' "$SCRIPT_DIR/prog.c" || fail "prog.c never names SHM_BADLEN"
