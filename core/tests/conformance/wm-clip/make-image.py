#!/usr/bin/env python3
"""core/tests/conformance/wm-clip/make-image.py — FAT16 with A.ELF + B.ELF."""
...
def main():
    if len(sys.argv) != 4:
        raise SystemExit("usage: make-image.py <out.img> <a.elf> <b.elf>")
    out, e1, e2 = sys.argv[1], sys.argv[2], sys.argv[3]
    order = ("A.ELF", "B.ELF")
    blobs = {"A.ELF": open(e1, "rb").read(), "B.ELF": open(e2, "rb").read()}
    if blobs["A.ELF"] == blobs["B.ELF"]:
        raise SystemExit("make-image: ELFs are byte-identical")
    ...
    for name in order:
        ...
        while len(chain) < need:
            if c >= CLUSTER_COUNT + 2:
                raise SystemExit("out of clusters for %s" % name)
            if c not in taken:
                chain.append(c)
                taken.add(c)
            c += 1
    ...
    for name in order:
        root.extend(dir_entry(eightthree(name), 0x20, chains[name][0], len(blobs[name])))
    ...
    print("make-image: wrote %s (%d bytes, A.ELF+B.ELF)" % (out, len(img)))
