#!/usr/bin/env bash
# core/tests/conformance/plat-libc/run.sh
#
# ADR-0152: a named platform ELF may dlopen OUR tiny FAT LIBC.SO,
# resolve write, and call it. TAP/FILES stay 64 KiB / 2 MiB. Same
# binary planted as PLAT.ELF and ASK.ELF — only the name honours
# syscall 29. PLAT calls write through mapped pages and writes the
# derived LINE; MISS.SO is NotFound. ASK.ELF of the same bytes is
# BadArg.
#
# Not glibc. Not CEF OnPaint. Not a 189 MiB map. Syscall 11 stays
# fdwait. No help line. drawRRect owns osgfx_skia. de-browse floor
# stays 87. Leftover: 189 MiB / the rest of DT_NEEDED.
#
# Usage: bash run.sh
...
fail() { echo "PLAT-LIBC: FAIL — $1" >&2; exit 1; }
setup_error() { echo "PLAT-LIBC: FAIL — $1" >&2; exit 2; }
...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-plat-libc.XXXXXX")" \
...
# Lean kernel: this door is ELF/FAT libc, not Graphite or FFmpeg.
...
ck; x86_64-elf-nm "$KERNEL_ELF" | grep -E '[[:space:]]T[[:space:]]+elfDlopenMakeExec$' \
  || fail "kernel.elf has no elfDlopenMakeExec — write call would stay NX"
...
ck; grep -q 'elfStrWrite' "$CORE_DIR/kernel/elf.dart" \
  || fail "elf.dart has no elfStrWrite — write resolve is missing"
ck; grep -q 'elfDlopenMakeExec' "$CORE_DIR/kernel/elf.dart" \
  || fail "elf.dart has no elfDlopenMakeExec"
ck; grep -q 'ADR-0152' "$CORE_DIR/docs/decisions/0152-tiny-libc-is-the-platform-posix-door.md" \
  || fail "ADR-0152 file is missing"
ck; grep -q '0148 is setfs' "$CORE_DIR/docs/decisions/0152-tiny-libc-is-the-platform-posix-door.md" \
  || fail "ADR-0152 stole 0148"
ck; ! grep -q 'proc libc\|libc ' "$CORE_DIR/kernel/shell.dart" \
  || fail "shell.dart grew a libc help string"
...
echo "STRUCTURAL: pass  libc write via dlopen=29 on PLAT.ELF only; TAP/FILES 64K/2MiB; fdwait=11; help 2511"
...
DISK_IMG="$WORKDIR/plat-libc.img"
...
KEYS="$KEYS,$(typekeys 'proc spawn plat.elf'),ret,until:LINE ,wait:400"
    ...
    def __init__(self, port):
        ...
        while time.time() < deadline:
            try:
                ...
                print("PLAT-LIBC: QEMU", hello.get("QMP", {}).get("version", {}))
                return
            except OSError as e:
                last = e
                time.sleep(0.2)
...
echo
echo "=== ASSERT ==="
python3 - "$SER" "$DERIVED" <<'PY' || fail "the boot does not satisfy the libc door"
import re, sys
...
need("FS OPEN LIBC    .SO", "libc.so opened")
need("VIA LIBC", "write called through mapped libc")
need(derived["line_line"], "derived line from libc write return")
...
if ser.count("LINE ") < 1:
    fails.append("no LINE — write was not called through mapped libc")
if ser.count(derived["line_line"]) != 1:
    fails.append("derived LINE count: %d, expected 1" % ser.count(derived["line_line"]))
...
if not opens:
    ...
    va = int(opens[0], 16)
    if va < 0x10600000 or va >= 0x17400000:
...
print("    (PLAT.ELF dlopen LIBC.SO → call write → derived LINE; MISS.SO NotFound; "
      "ASK.ELF same bytes refused)")
PY

require_assertions "$ASSERTIONS_REQUIRED"
echo "PLAT-LIBC: PASS — named PLAT.ELF dlopen'd LIBC.SO (syscall 29); write returned MARK and printed derived LINE; MISS.SO is NotFound; ASK.ELF is the same bytes and is refused; TAP/FILES stay 64K/2MiB; leftovers rest of DT_NEEDED / OnPaint"
exit 0
