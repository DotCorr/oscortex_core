#!/usr/bin/env python3
"""FAT16 volume: identical PLAT.ELF / ASK.ELF plus LIBC.SO.

Only the 8.3 name may honour dlopen (ADR-0152). LIBC.SO is OUR
tiny libc. MISS.SO is deliberately absent.
"""
...
def main():
    if len(sys.argv) != 4:
        raise SystemExit("usage: make-image.py <out.img> <plat.elf> <libc.so>")
    ...
    blobs = {"PLAT.ELF": blob, "ASK.ELF": blob, "LIBC.SO": so}
    ...
    chains = {}
    c = 2
    for name in ("PLAT.ELF", "ASK.ELF", "LIBC.SO"):
    ...
    print("make-image: %s — PLAT/ASK %d bytes, LIBC.SO %d (MISS.SO absent)"
