...
#     calls syscall 16 and reports the packed value both by writing it and by
...
echo "CHECK 8: pass  A PROGRAM AT CPL 3 READ THE POINTER — syscall 16 returned $(dm packed), the program took it apart on its own side of the boundary into X $(dm final_x) Y $(dm final_y) B $(dm final_b) N $(dm count_field), and exited with $(dm exit_code), a number computed on the host before the boot"
