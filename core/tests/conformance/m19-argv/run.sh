...
# M20 (ADR-0027): THE SAME PROPERTY, CHECKED WHERE IT NOW LIVES. `run` no
...
grep -q "enter_user(" "$CORE_DIR/kernel/elf.dart" \
  && fail "elf.dart enters ring 3 again — ADR-0027 made procCreate/procStart the only launch path, and a program launched from elf.dart has no process slot and therefore no heap"
grep -q "final u64 rsp = argsBuild(elfMeta(u64(elfMetaStackFrame)));" \
  "$CORE_DIR/kernel/proc.dart" \
  || fail "procCreate does not build the initial stack in the frame the loader mapped at vmProgStackPage (ADR-0027 moved this line out of elf.dart; it must be in exactly one place)"
