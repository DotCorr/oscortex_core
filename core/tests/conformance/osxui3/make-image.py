#!/usr/bin/env python3
"""core/tests/conformance/osxui3/make-image.py

OSCXPRG1 disk so `proc spawn <lba>` can start MENU.ELF (and the
...
m10-elf invented.
    ...
    make-image.py <out.img> <menu.elf> <nocom.elf> [--json]
...
def verify(path, paths, layout):
    ...
    a = blob[layout["MENU"]["image_lba"] * SECTOR:
             layout["MENU"]["image_lba"] * SECTOR + layout["MENU"]["bytes"]]
    b = blob[layout["NOCOM"]["image_lba"] * SECTOR:
             layout["NOCOM"]["image_lba"] * SECTOR + layout["NOCOM"]["bytes"]]
    if a == b:
        fails.append("MENU and NOCOM are byte-identical on the disk")
...
def main():
    ...
    if want_json:
        ...
        print("make-image: MENU at LBA 0x%X (%d bytes), NOCOM at LBA 0x%X (%d bytes)"
              % (layout["MENU"]["header_lba"], layout["MENU"]["bytes"],
