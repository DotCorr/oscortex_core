#!/usr/bin/env bash
# core/tests/conformance/plat-need3/run.sh
#
# ADR-0162: named PLAT.ELF walks eight DT_NEEDED (LIBC.SO + LIBM.SO
# + LIBDL.SO + LIBPT.SO + LIBGB.SO + LIBGO.SO + LIBNP.SO + LIBNS.SO),
# dlopens each, and prints derived LINE1..LINE8 from write / need_fn
# / dl_fn / pt_fn / gb_fn / go_fn / np_fn / ns_fn. ASK.ELF of the
# same bytes is REFUSED 11. A volume without LIBNS.SO cannot invent
# LINE8. Satisfies 8 of 32 CEF DT_NEEDED stand-ins; 24 remain.
# OnPaint leftover. Not glibc. Not libcef. Syscall 11 stays fdwait.
# TAP/FILES stay 64K/2MiB. No help line. Graphite / osgfx_skia /
...
fail() { echo "PLAT-NEED3: FAIL — $1" >&2; exit 1; }
setup_error() { echo "PLAT-NEED3: FAIL — $1" >&2; exit 2; }
...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-plat-need3.XXXXXX")" \
...
ck; grep -aob $'go_fn\0' "$KERNEL_ELF" >/dev/null \
  || fail "kernel.elf has no go_fn NUL string — ADR-0162 resolve is missing"
ck; grep -aob $'np_fn\0' "$KERNEL_ELF" >/dev/null \
  || fail "kernel.elf has no np_fn NUL string — ADR-0162 resolve is missing"
...
ck; grep -q 'elfStrGbFn' "$CORE_DIR/kernel/elf.dart" \
  || fail "elf.dart has no elfStrGbFn — gb_fn resolve is missing"
ck; grep -q 'elfStrNsFn' "$CORE_DIR/kernel/elf.dart" \
  || fail "elf.dart has no elfStrNsFn — ns_fn resolve is missing"
ck; grep -q 'ADR-0162' "$CORE_DIR/docs/decisions/0162-dt-needed-loads-eight-fat-sos.md" \
  || fail "ADR-0162 file is missing"
ck; grep -q '0161 is' "$CORE_DIR/docs/decisions/0162-dt-needed-loads-eight-fat-sos.md" \
  || fail "ADR-0162 stole 0161"
...
echo "STRUCTURAL: pass  DT_NEEDED×8 via dlopen=29; TAP/FILES 64K/2MiB; fdwait=11; help 2511; 8/32 satisfied"
...
DISK_FULL="$WORKDIR/plat-need3.img"
DISK_MISS="$WORKDIR/plat-need3-miss.img"
...
ck; python3 "$SCRIPT_DIR/make-image.py" "$DISK_MISS" "$WORKDIR/plat.elf" \
  "$WORKDIR/libc.so" "$WORKDIR/libm.so" "$WORKDIR/libdl.so" "$WORKDIR/libpt.so" \
  "$WORKDIR/libgb.so" "$WORKDIR/libgo.so" "$WORKDIR/libnp.so" \
  || fail "make-image.py could not write the miss-libns volume"
    ...
    def __init__(self, port):
        ...
        while time.time() < deadline:
            try:
                ...
                print("PLAT-NEED3: QEMU", hello.get("QMP", {}).get("version", {}))
                return
            except OSError as e:
                last = e
                time.sleep(0.2)
...
echo
echo "=== BOOT — full (8 DT_NEEDED) then ASK.ELF ==="
KEYS_FULL="$(typekeys 'vm'),ret,until:READY 1"
KEYS_FULL="$KEYS_FULL,$(typekeys 'proc spawn plat.elf'),ret,until:LINE8 ,wait:600"
...
echo
echo "=== BOOT — miss LIBNS.SO ==="
KEYS_MISS="$(typekeys 'vm'),ret,until:READY 1"
KEYS_MISS="$KEYS_MISS,$(typekeys 'proc spawn plat.elf'),ret,until:MISS ,wait:600"
run_boot miss "$DISK_MISS" "$KEYS_MISS" "$WORKDIR/miss/serial.txt"

echo
echo "=== ASSERT ==="
python3 - "$WORKDIR/full/serial.txt" "$WORKDIR/miss/serial.txt" "$DERIVED" <<'PY' || fail "boots do not satisfy the eight-DT_NEEDED door"
import re, sys
...
need(full, "NEED3 START", "program ran")
...
need(full, "FS OPEN LIBGB   .SO", "libgb.so opened")
need(full, "FS OPEN LIBGO   .SO", "libgo.so opened")
need(full, "FS OPEN LIBNP   .SO", "libnp.so opened")
need(full, "FS OPEN LIBNS   .SO", "libns.so opened")
...
need(full, "VIA LIBGB", "gb_fn called through libgb")
need(full, "VIA LIBGO", "go_fn called through libgo")
need(full, "VIA LIBNP", "np_fn called through libnp")
need(full, "VIA LIBNS", "ns_fn called through libns")
for key in ("line1", "line2", "line3", "line4", "line5", "line6", "line7", "line8"):
    need(full, derived[key], "derived " + key.upper())
need(full, derived["need_eight"], "all eight NEEDED loaded")
...
if len(opens) < 8:
    fails.append("PROC DLOPEN VA count: %d, expected ≥8" % len(opens))
else:
    for va_s in opens[:8]:
        va = int(va_s, 16)
...
# Anti-vacuity: missing LIBNS.SO → LINE1..7 ok, no LINE8.
need(miss, "NEED3 START", "miss program ran")
...
need(miss, "FS OPEN LIBPT   .SO", "miss libpt opened")
need(miss, "FS OPEN LIBGB   .SO", "miss libgb opened")
need(miss, "FS OPEN LIBGO   .SO", "miss libgo opened")
need(miss, "FS OPEN LIBNP   .SO", "miss libnp opened")
for key in ("line1", "line2", "line3", "line4", "line5", "line6", "line7"):
    need(miss, derived[key], "miss " + key.upper())
need(miss, derived["miss_line"], "miss missing LIBNS refused")
need(miss, derived["need_seven"], "miss stopped after seven NEEDED")
if derived["line8"] in miss:
    fails.append("miss invented LINE8 without LIBNS.SO")
if "FS OPEN LIBNS   .SO" in miss:
    fails.append("miss opened LIBNS.SO — volume should lack it")
if "VIA LIBNS" in miss:
    fails.append("miss called through LIBNS without the file")
if "ERR FFFFFFFFFFFFFFF9" not in miss:
    fails.append("miss never printed NotFound")

if derived["satisfied"] != "8" or derived["remain"] != "24":
    fails.append("derive count wrong: satisfied=%s remain=%s" % (
        derived["satisfied"], derived["remain"]))
remain = derived.get("remaining", "").split(",")
if len(remain) != 24:
    fails.append("remaining list length %d, expected 24" % len(remain))
for must in ("libnssutil3.so", "libX11.so.6", "ld-linux-x86-64.so.2"):
...
if fails:
    ...
    print("\n".join(full.splitlines()[-120:]), file=sys.stderr)
    print("---- miss serial (tail) ----", file=sys.stderr)
    print("\n".join(miss.splitlines()[-120:]), file=sys.stderr)
    sys.exit(1)

print("    (PLAT.ELF DT_NEEDED×8 → eight FAT stand-ins derived LINE1..8; "
      "LIBNS absent → no LINE8; ASK.ELF REFUSED 11; 8/32 CEF NEEDED stand-ins; "
      "24 remain)")
PY

require_assertions "$ASSERTIONS_REQUIRED"
echo "PLAT-NEED3: PASS — named PLAT.ELF walked 8 DT_NEEDED (LIBC.SO + LIBM.SO + LIBDL.SO + LIBPT.SO + LIBGB.SO + LIBGO.SO + LIBNP.SO + LIBNS.SO); derived LINE1..LINE8; missing LIBNS.SO refuses LINE8; ASK.ELF REFUSED 11; satisfied 8/32 CEF NEEDED stand-ins (24 remain); TAP/FILES 64K/2MiB; leftover OnPaint + 24 DT_NEEDED"
exit 0
