...
ck; if [[ "$NON_PMM_BSS" -ne 440 ]]; then
  fail "the kernel holds $(( KDATA_BSS + ASM_BSS )) bytes of mutable static storage, of which $PMM_STORE_SIZE are M7's pmmStore and $VM_STORE_SIZE are M8's vmStore, leaving $NON_PMM_BSS — expected 440 (M5's 424, plus the 16 bytes ADR-0064's geometry words added to fbStateBlock). ATA PIO was supposed to add NONE. A 512-byte sector buffer is exactly what this driver does not have; if you meant to grow it, say so in kdata.S's header, in GAP-0053, and in docs/decisions/0010-ata-pio-disk-read.md."
fi
echo "STRUCTURAL: pass  440 bytes of mutable static storage outside M7's page-allocator and M8's page-table blocks — the disk driver still adds ZERO (no sector buffer exists)"
