#!/usr/bin/env python3
"""core/tests/conformance/de-resize/make-image.py

FAT16 volume with one 8.3 ELF: WIN.ELF. Geometry matches de-wm /
de-chrome so fsck_msdos accepts it.

    make-image.py <out.img> <win.elf>
"""
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("usage: make-image.py <out.img> <win.elf>")
    out, elf1 = sys.argv[1], sys.argv[2]

    blob = open(elf1, "rb").read()
    if not blob:
        raise SystemExit("make-image: empty WIN.ELF")
    ...
    while len(chain) < need:
        if c >= CLUSTER_COUNT + 2:
            raise SystemExit("make-image: ran out of clusters placing WIN.ELF")
        if c not in taken:
            chain.append(c)
            taken.add(c)
        c += 1
    ...
    add("WIN.ELF", 0x20, chain[0], len(blob))
    ...
    if joined[:len(blob)] != blob:
        raise SystemExit("make-image: WIN.ELF does not read back")

    print("make-image: %s — WIN.ELF %d" % (os.path.basename(out), len(blob)))
