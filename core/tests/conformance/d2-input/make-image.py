...
told TWO sector numbers instead of one.
    ...
    make-image.py <out.img> <progR.elf> <progE.elf> [--json]
...
def verify(path, paths, layout):
    ...
    a = blob[layout["R"]["image_lba"] * SECTOR:
             layout["R"]["image_lba"] * SECTOR + layout["R"]["bytes"]]
    b = blob[layout["E"]["image_lba"] * SECTOR:
             layout["E"]["image_lba"] * SECTOR + layout["E"]["bytes"]]
    if a == b:
...
def main():
    if len(sys.argv) < 4:
        ...
        sys.exit(2)
    out = sys.argv[1]
    paths = {"R": sys.argv[2], "E": sys.argv[3]}
