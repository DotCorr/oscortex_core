...
def main():
    ...
    out, elf_path, so_path = sys.argv[1], sys.argv[2], sys.argv[3]
    blob = open(elf_path, "rb").read()
    so = open(so_path, "rb").read()
    if len(blob) < 64 or len(so) < 64:
        raise SystemExit("make-image: ELF/.so is too small")
    if len(blob) > 65536 or len(so) > 65536:
    ...
    chains = {}
    c = 2
    for name in ("PLAT.ELF", "ASK.ELF", "CEF.SO"):
    ...
    print("make-image: %s — PLAT/ASK %d bytes, CEF.SO %d (MISS.SO absent)"
          % (os.path.basename(out), len(blob), len(so)))
