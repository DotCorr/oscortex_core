#!/usr/bin/env python3
"""core/tests/conformance/m20-ipc/make-image.py

Writes M20's test disk: the SAME 32-byte header-sector format m10-elf invented,
...
only place that could notice.
    ...
    make-image.py <out.img> <ipc.elf> [--json]
...
def main():
    ...
    if len(args) != 2:
        raise SystemExit("usage: make-image.py <out.img> <ipc.elf> [--json]")
    out, elf = args
    ...
    if ranges["A"] != ranges["B"]:
        raise SystemExit("make-image: the two program slots are NOT byte-identical -- "
                         "M20's claim is that ONE binary takes two roles, and this "
