#!/usr/bin/env bash
# core/tests/conformance/shm-multi/run.sh
#
# ADR-0158 — multi-mapper grow and shrink on one shm.
# docs/decisions/0158-two-mappers-grow-and-shrink.md
#
# One ELF in two slots (`proc coop`). Producer creates 3 pages, grants,
# peer maps RO (MAPS 0002). Producer grows to 6; peer must read the new
# marks. Producer shrinks to 2; peer probe of a freed page must #PF.
...
fail() { echo "SHM-MULTI: FAIL — $1" >&2; exit 1; }
setup_error() { echo "SHM-MULTI: FAIL — $1" >&2; exit 2; }
...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-shm-multi.XXXXXX")" \
...
ck; [[ -f "$CORE_DIR/docs/decisions/0158-two-mappers-grow-and-shrink.md" ]] \
  || fail "ADR-0158 is missing"
...
echo
echo "=== STRUCTURAL ==="
ck; grep -q 'shmMapRangeAll' "$SHM_SRC" \
  || fail "shmMapRangeAll is missing"
ck; grep -q 'shmUnmapRangeAll' "$SHM_SRC" \
  || fail "shmUnmapRangeAll is missing"
ck; ! grep -q 'shmRegMaps)) > u64(1)' "$SHM_SRC" \
  || fail "alone-mapper refuse still present in shm.dart"
...
ck; ! grep -q 'SYS_SHMGROW' "$CORE_DIR/user/libc/oslibc.h" \
  || fail "oslibc.h grew SYS_SHMGROW"
ck; ! grep -q 'SYS_SHMSHRINK' "$CORE_DIR/user/libc/oslibc.h" \
  || fail "oslibc.h grew SYS_SHMSHRINK"
...
echo "STRUCTURAL: pass  multi-mapper grow/shrink 34/35 fdwait=11 help=2511"
...
DISK_IMG="$WORKDIR/shm-multi.img"
...
echo
echo "=== BOOT — proc coop two mappers ==="
...
echo
echo "=== ASSERT ==="
ck; grep -qE 'SHM MAP .* MAPS 0002' "$SER" \
  || { sed -n '/M1 END/,$p' "$SER" >&2; fail "missing SHM MAP … MAPS 0002 (two mappers)"; }
ck; grep -qE 'SHM GROW R [0-9] PAGES 0006 .*MAPS 0002' "$SER" \
  || { sed -n '/M1 END/,$p' "$SER" >&2; fail "missing SHM GROW … 0006 MAPS 0002"; }
ck; grep -qE 'SHM SHRINK R [0-9] PAGES 0002 .*MAPS 0002' "$SER" \
  || { sed -n '/M1 END/,$p' "$SER" >&2; fail "missing SHM SHRINK … 0002 MAPS 0002"; }
ck; grep -qF 'SHM MULTI PEER GROW OK' "$SER" \
  || { sed -n '/M1 END/,$p' "$SER" >&2; fail "peer did not read grown pages"; }
ck; grep -qF 'SHM MULTI PEER SHRINK PROBE' "$SER" \
  || { sed -n '/M1 END/,$p' "$SER" >&2; fail "missing peer shrink probe"; }
ck; ! grep -qF 'MULTI PEER STILL MAPPED' "$SER" \
  || fail "peer freed page still mapped — shrink was vacuous for peer"
ck; ! grep -qF 'MULTI STILL MAPPED' "$SER" \
  || fail "owner freed page still mapped — shrink was vacuous"
ck; ! grep -qF 'MULTI PEER GROW BAD' "$SER" || fail "peer grow marks bad"
ck; ! grep -qF 'MULTI GROW FAIL' "$SER" || fail "grow failed under two mappers"
ck; ! grep -qF 'MULTI SHRINK FAIL' "$SER" || fail "shrink failed under two mappers"
ck; grep -qE 'USER FAULT VEC 0E' "$SER" \
  || { sed -n '/SHM MULTI PEER SHRINK PROBE/,$p' "$SER" >&2; fail "probe of freed page did not fault"; }
ck; python3 - "$SER" <<'PY' || fail "ordering wrong: peer grow then shrink probe then fault"
import sys
t = open(sys.argv[1]).read()
i_grow = t.find('SHM MULTI PEER GROW OK')
i_pr = t.find('SHM MULTI PEER SHRINK PROBE')
i_pf = t.find('USER FAULT VEC 0E')
if not (i_grow >= 0 and i_pr > i_grow and i_pf > i_pr):
    raise SystemExit('grow=%d probe=%d pf=%d' % (i_grow, i_pr, i_pf))
PY
echo "ASSERT: pass  two mappers; grow+shrink; peer read; freed page faults"

require_assertions "$ASSERTIONS_REQUIRED"
echo "SHM-MULTI: PASS — two mappers grow and shrink on one shm (ADR-0158)"
