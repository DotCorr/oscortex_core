#!/usr/bin/env bash
# core/tests/conformance/shm-multi/build-progs.sh
#
# Freestanding PROG for ADR-0158 (no oslibc — shmgrow/shmshrink unnamed).
...
grep -q 'SYS_SHMGROW' "$SCRIPT_DIR/prog.c" || fail "prog.c never names SYS_SHMGROW"
grep -q 'SYS_SHMSHRINK' "$SCRIPT_DIR/prog.c" || fail "prog.c never names SYS_SHMSHRINK"
