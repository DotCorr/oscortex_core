#!/usr/bin/env bash
# core/tests/conformance/de-graphite3/run.sh
#
# ADR-0153 — Chrome rrect is Graphite drawRRect.
# docs/design/c-modules.md, GAP-0313 leftover after ADR-0134.
#
# WHAT THIS ASSERTS
# ---------------------------------------------------------------------------
# Venus arms MakeVulkan. A chrome title-bar-class rrect is painted by
# Graphite drawRRect (not CPU drawRect spans). Serial
# OSGFX GRAPHITE RRECT 00C45A20. Homebrew stays NONE / no RRECT.
# Clear PIX alone is not enough.
#
# Usage: bash run.sh
...
fail() { echo "DE-graphite3: FAIL — $1" >&2; exit 1; }
setup_error() { echo "DE-graphite3: FAIL — $1" >&2; exit 2; }
...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-de-graphite3.XXXXXX")" \
...
ADR="$CORE_DIR/docs/decisions/0153-chrome-rrect-is-graphite-drawrrect.md"
ADR134="$CORE_DIR/docs/decisions/0134-graphite-makevulkan-needs-a-vkdevice.md"
GRAPHITE_LIB="$CORE_DIR/build/skia/out/guest-elf-graphite/libskia.a"
XRES=1200
YRES=720

echo "=== STRUCTURAL ==="
ck; [[ -f "$ADR" ]] || fail "ADR-0153 is missing"
ck; grep -q '0148 is setfs' "$ADR" || fail "ADR-0153 lost 0148 mention"
ck; [[ -f "$ADR134" ]] || fail "ADR-0134 was removed"
...
ck; grep -q 'graphite-rrect-gpu' "$GRAPHITE_GUEST" \
  || fail "graphite-rrect-gpu token missing"
ck; grep -q 'OSGFX GRAPHITE RRECT' "$GRAPHITE_GUEST" \
  || fail "RRECT serial line missing"
ck; grep -q '0x00C45A20' "$GRAPHITE_GUEST" \
  || fail "Graphite chrome rrect colour missing"
ck; grep -q 'drawRRect' "$GRAPHITE_GUEST" \
  || fail "Graphite path does not call drawRRect"
ck; grep -q 'osgfx_vk_queue_rrect' "$GRAPHITE_GUEST" \
  || fail "Graphite path does not queue ICD rrect for DRAW"
ck; grep -q 'osgfx_vk_queue_rrect' "$VK_C" \
  || fail "ICD has no queue_rrect"
ck; grep -q 'OSGFX_VK_OP_DRAW' "$VK_C" \
  || fail "ICD DRAW op missing"
ck; grep -q 'img_fill_rrect' "$VK_C" \
  || fail "ICD DRAW does not fill rrect coverage"
ck; grep -q 'osgfx_graphite_fill_rrect' "$SKIA_CPP" \
  || fail "osgfx_skia.cpp does not call Graphite fill_rrect"
# The Graphite arm inside osgfx_fill_rect must RETURN once it has painted,
# never fall through to the CPU put_px loop below it. Read the arm into a
# variable rather than piping into `grep -q`: the arm has grown past the pipe
# buffer, and `grep -q` exiting early SIGPIPEs awk, which pipefail then
# reports as a failure that has nothing to do with the code under test.
GRAPHITE_ARM=$(awk '/osgfx_graphite_ready\(\) != 0/,/draw_rrect_spans/' "$SKIA_CPP")
ck; [[ -n "$GRAPHITE_ARM" ]] \
  || fail "osgfx_skia.cpp has no osgfx_graphite_ready arm at all"
ck; printf '%s\n' "$GRAPHITE_ARM" | grep -q 'return;' \
  || fail "Graphite title path still falls through to CPU drawRect"
# Stronger than the bare grep: the arm must return on the path that actually
# painted, i.e. immediately after a successful osgfx_graphite_fill_* call.
ck; printf '%s\n' "$GRAPHITE_ARM" \
  | awk '/osgfx_graphite_fill_[a-z_]+\(.*\) != 0\) \{/{hit=1; next}
         hit && /return;/{ok=1; hit=0}
         END{exit ok?0:1}' \
  || fail "a successful Graphite fill does not return — it falls through to put_px"
...
ck; ! grep -q 'de-graphite3\|GRAPHITE RRECT\|0149' \
      "$CORE_DIR/docs/syscall-registry.md" \
  || fail "ADR-0153 added a syscall"
echo "STRUCTURAL: pass  ADR-0153; drawRRect; ICD DRAW rrect; no CPU fallback"
...
echo "ANTI-VACUITY: pass  CPU-only has no Graphite rrect token"
...
ck; elf_has "$KERNEL_ELF" "graphite-rrect-gpu" \
  || fail "kernel.elf has no graphite-rrect-gpu"
...
echo "GRAPHITE LINK: pass  rrect token + MakeVulkan + ICD"
...
echo "HOMEBREW: pass  NONE path; no RRECT; no PIX; WM GFX ON"
...
# Match de-graphite2: no fb on virtio-gl (FB NONE). RRECT proof is in
# graphite_try beside PIX — does not need sit-in scanout.
GL_KEYS="$(typekeys 'virtgpuv'),ret,wait:8000"
GL_KEYS="$GL_KEYS,$(typekeys 'wm gfx'),ret,wait:15000"
...
echo "VENUS: pass  OK; PIX; RRECT 00C45A20; WM GFX ON"

require_assertions "$ASSERTIONS_REQUIRED"
echo "DE-graphite3: PASS — Graphite chrome rrect (not CPU drawRect); Homebrew NONE ($ASSERTIONS checks)"
exit 0
