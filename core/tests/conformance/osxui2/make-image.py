#!/usr/bin/env python3
"""core/tests/conformance/osxui2/make-image.py

OSCXPRG1 disk so `proc spawn <lba>` can start BTN.ELF. Same 32-byte
header-sector format m10-elf invented.

    +0   "OSCXPRG1"
    +8   u64       the image's length in bytes
    +16  u64       the LBA its first sector is at

    make-image.py <out.img> <btn.elf> [--json]
...
def main():
    ...
    if want_json:
        ...
        print("make-image: BTN at LBA 0x%X (%d bytes)"
              % (layout["BTN"]["header_lba"], layout["BTN"]["bytes"]))
