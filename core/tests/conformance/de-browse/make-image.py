#!/usr/bin/env python3
"""core/tests/conformance/de-browse/make-image.py

FAT16 volume with BROWSE.ELF and NONE.ELF so hidden `go` can start
each twin. Geometry matches de-apps so fsck_msdos accepts it.

    make-image.py <out.img> <browse.elf> <ninit.elf>
"""
import os
import struct
import sys
...
def main():
    if len(sys.argv) != 4:
        raise SystemExit("usage: make-image.py <out.img> <browse.elf> <ninit.elf>")
    out, elf1, elf2 = sys.argv[1], sys.argv[2], sys.argv[3]

    blobs = {
        "BROWSE.ELF": open(elf1, "rb").read(),
        "NINIT.ELF": open(elf2, "rb").read(),
    }
    if blobs["BROWSE.ELF"][:4] != b"\x7fELF":
        raise SystemExit("make-image: BROWSE.ELF is not ELF")
    if blobs["NINIT.ELF"][:4] != b"\x7fELF":
        raise SystemExit("make-image: NINIT.ELF is not ELF")
    if blobs["BROWSE.ELF"] == blobs["NINIT.ELF"]:
        raise SystemExit("make-image: the two ELFs are byte-identical")

    taken = set()
    chains = {}
    c = 2
    for name in ("BROWSE.ELF", "NINIT.ELF"):
    ...
    add("BROWSE.ELF", 0x20, chains["BROWSE.ELF"][0], len(blobs["BROWSE.ELF"]))
    add("NINIT.ELF", 0x20, chains["NINIT.ELF"][0], len(blobs["NINIT.ELF"]))
    ...
    print("make-image: %s — BROWSE.ELF %d, NINIT.ELF %d"
          % (os.path.basename(out), len(blobs["BROWSE.ELF"]),
             len(blobs["NINIT.ELF"])))
