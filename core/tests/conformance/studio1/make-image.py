#!/usr/bin/env python3
"""core/tests/conformance/studio1/make-image.py

FAT16 volume for the STUDIO1 listing: STUDIO.ELF and a planted catalog
APPS.TXT. Geometry matches apps1 / frame1 so fsck_msdos accepts it.

    make-image.py <out.img> <studio.elf> [--json] [--variant=trunc]
"""
...
# Two names, one per line, no trailing blank. A one-name catalog would
# let a harness that only greps APP1.ELF pass if the program baked it.
CATALOG = b"APP1.ELF\nAPP2.ELF\n"
CATALOG_TRUNC = b"APP2.ELF\n"
...
def main():
    ...
    for a in sys.argv[1:]:
        if a.startswith("--variant="):
            variant = a.split("=", 1)[1]
    if len(args) != 2:
        raise SystemExit("usage: make-image.py <out.img> <studio.elf> [--variant=trunc]")
    out, elf = args
    if variant not in ("full", "trunc"):
        raise SystemExit("make-image: unknown variant %r" % variant)

    catalog = CATALOG_TRUNC if variant == "trunc" else CATALOG
    blobs = {
        "STUDIO.ELF": open(elf, "rb").read(),
        "APPS.TXT": catalog,
    }
    if variant == "full":
        if b"APP1.ELF" not in catalog or b"APP2.ELF" not in catalog:
            raise SystemExit("make-image: full catalog does not list both apps")
        if catalog.count(b"\n") < 2:
            raise SystemExit("make-image: full catalog must list two names")
    else:
        if b"APP1.ELF" in catalog:
            raise SystemExit("make-image: trunc catalog still lists APP1.ELF")
        if b"APP2.ELF" not in catalog:
            raise SystemExit("make-image: trunc catalog lost APP2.ELF")

    taken = set()
    chains = {}
    c = 2
    for name in ("STUDIO.ELF", "APPS.TXT"):
    ...
    if want_json:
        ...
        print("make-image: %s — STUDIO.ELF %d, APPS.TXT %d (%s)"
              % (os.path.basename(out), len(blobs["STUDIO.ELF"]),
                 len(blobs["APPS.TXT"]), variant))
