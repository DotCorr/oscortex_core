#!/usr/bin/env python3
"""core/tests/conformance/frame3/make-image.py

FAT16 volume for FRAME3: SURF.ELF (kbdevent + persist) and NOKBD.ELF
(the colour negative). Geometry matches frame1 / apps1 / m15-fileio
so fsck_msdos accepts it. THEME.DAT is not planted — the guest creates it.

    make-image.py <out.img> <surf.elf> <nokbd.elf> [--json]
"""
...
def main():
    ...
    if len(args) != 3:
        raise SystemExit("usage: make-image.py <out.img> <surf.elf> <nokbd.elf>")
    out, elf1, elf2 = args

    blobs = {
        "SURF.ELF": open(elf1, "rb").read(),
        "NOKBD.ELF": open(elf2, "rb").read(),
    }
    if blobs["SURF.ELF"] == blobs["NOKBD.ELF"]:
        raise SystemExit("make-image: SURF.ELF and NOKBD.ELF are byte-identical")
    if "THEME.DAT" in blobs:
        raise SystemExit("make-image: THEME.DAT must not be planted")

    taken = set()
    chains = {}
    c = 2
    for name in ("SURF.ELF", "NOKBD.ELF"):
    ...
    add("SURF.ELF", 0x20, chains["SURF.ELF"][0], len(blobs["SURF.ELF"]))
    add("NOKBD.ELF", 0x20, chains["NOKBD.ELF"][0], len(blobs["NOKBD.ELF"]))
    ...
    if want_json:
        ...
        print("make-image: %s — SURF.ELF %d, NOKBD.ELF %d, no THEME.DAT"
              % (os.path.basename(out), len(blobs["SURF.ELF"]),
                 len(blobs["NOKBD.ELF"])))
