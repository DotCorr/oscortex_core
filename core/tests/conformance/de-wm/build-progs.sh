#!/usr/bin/env bash
# core/tests/conformance/de-wm/build-progs.sh
#
# WIN.ELF — resident window for title-drag / panel.
...
grep -q 'DE WM COMMIT' "$SCRIPT_DIR/win.c" \
  || fail "win.c does not contain DE WM COMMIT"
