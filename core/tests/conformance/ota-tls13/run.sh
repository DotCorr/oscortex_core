#!/usr/bin/env bash
# core/tests/conformance/ota-tls13/run.sh
#
# ADR-0177 — OTA signed blob fetched from a real host over TLS 1.3.
# docs/decisions/0177-ota-tls13-record-layer.md.
#
# WHAT THIS ASSERTS
# ---------------------------------------------------------------------------
...
# `ota tls13 <port>` pulls the OTA1 blob through the record layer, verifies,
# and applies.
#
...
fail() { echo "OTA-TLS13: FAIL — $1" >&2; exit 1; }
setup_error() { echo "OTA-TLS13: FAIL — $1" >&2; exit 2; }
...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-ota-tls13.XXXXXX")" || setup_error "mktemp failed"
...
ck; grep -q 'void otaTls13()' "$OTA_SRC" \
...
ck; grep -q 'shellStartsWith(Rodata.addressOf(otaStrCmdTls13)' "$CORE_DIR/kernel/shell.dart" \
  || fail "shell.dart does not dispatch ota tls13 before ota tls"
ck; grep -q '0x1301\|build_client_hello_13\|otatls_tick_13' "$CORE_DIR/plat/otatls/otatls13.inc" \
  || fail "otatls13.inc has no TLS 1.3 AES-GCM suite"
...
ck; [[ -f "$CORE_DIR/docs/decisions/0177-ota-tls13-record-layer.md" ]] \
  || fail "ADR-0177 missing"
...
echo "STRUCTURAL: pass  otaTls13+0x1301+GCM, otatls linked@32960, fdwait, tls13 before tls"
...
boot_ota() {
  ...
  cp "$ser" "$CORE_DIR/build/ota-tls13-$label-serial.txt"
  cp "$img" "$CORE_DIR/build/ota-tls13-$label.img"
}
...
print("HOST-SELF-TLS13: got %d bytes on port %d cipher=%s" % (len(got), port, "TLS_AES_128_GCM_SHA256"))
PY
GET_GOOD_KEYS="$(typekeys "ota tls13 $GOOD_PORT"),ret,wait:90000,$(typekeys "cat slot.txt"),ret,wait:1500"
...
GET_BAD_KEYS="$(typekeys "ota tls13 $BAD_PORT"),ret,wait:20000,$(typekeys "cat slot.txt"),ret,wait:1500"
...
GET_BC_KEYS="$(typekeys "ota tls13 $BC_PORT"),ret,wait:20000,$(typekeys "cat slot.txt"),ret,wait:1500"
...
GET_NONE_KEYS="$(typekeys "ota tls13 $CLOSED_PORT"),ret,wait:12000"
...
echo
echo "=== BOOT tls12-only anti-vacuity (must not accept TLS 1.2 on tls13 door) ==="
start_tls12_listener() {
  local blob="$1"
  local cert="$2"
  local key="$3"
  local portfile="$4"
  ...
  rm -f "$portfile"
  : >"$logfile"
  python3 - "$blob" "$cert" "$key" "$portfile" "$logfile" "$PICKER" <<'PY12' &
import ssl, socket, sys, time, subprocess
blob_path, cert, key, portfile, logfile, picker = sys.argv[1:7]
port = int(subprocess.check_output(["python3", picker], text=True).strip())
blob = open(blob_path, "rb").read()
...
PY12
  ...
  [[ -s "$portfile" ]] || fail "tls12 listener did not publish a port"
}
start_tls12_listener "$WORKDIR/blob.bin" "$WORKDIR/good-cert.pem" \
  "$WORKDIR/good-key.pem" "$WORKDIR/tls12.port" "$WORKDIR/tls12-listen.log"
TLS12_PORT=$(tr -d '[:space:]' < "$WORKDIR/tls12.port")
GET_TLS12_KEYS="$(typekeys "ota tls13 $TLS12_PORT"),ret,wait:20000"
boot_ota tls12 "$GET_TLS12_KEYS"
stop_listener
SER_TLS12="$WORKDIR/tls12/serial.txt"
ck; ! grep -q 'OTA OK' "$SER_TLS12" \
  || fail "TLS 1.2-only listener produced OTA OK — TLS 1.3 was stubbed"
ck; grep -qE 'OTA (TLSFAIL|NOHOST|BADCERT|BADMAGIC|SHORT)' "$SER_TLS12" \
  || fail "TLS 1.2-only boot did not refuse without OTA OK"
SLOT_TLS12=$(read_slot "$WORKDIR/tls12.img" | head -n1)
ck; [[ "$SLOT_TLS12" == "$OLD_HEX" ]] \
  || fail "host slot after TLS 1.2 anti-vacuity is $SLOT_TLS12, want OLD!"
echo "ASSERT: pass  TLS 1.2-only port is not accepted as TLS 1.3"
...
GET_PLAIN_KEYS="$(typekeys "ota tls13 $PLAIN_PORT"),ret,wait:20000"
...
echo
echo "OTA-TLS13: PASS — TLS 1.3 AES-128-GCM fetch applies signed blob; bad sig / bad cert / no listener / TLS 1.2-only / plain TCP refuse; leftover is Wi-Fi (GAP-0323)"
exit 0
