#!/usr/bin/env python3
"""FAT16 volume for plat-dyn.

Full image: PLAT.ELF + ASK.ELF (identical dyn bytes) + LD.SO.
No-interp image: PLAT.ELF only — the anti-vacuity disk.
"""
...
def main():
    ...
    extra = " + LD.SO %d bytes" % len(blobs["LD.SO"]) if "LD.SO" in blobs else " (no LD.SO)"
    print("make-image: %s — PLAT.ELF/ASK.ELF %d bytes%s"
          % (os.path.basename(out), len(blob), extra))
