#!/usr/bin/env python3
"""core/tests/conformance/de-apps/make-image.py

FAT16 volume for the closed app contract: TAP.ELF in the root so
hidden `go TAP.ELF` (syscall 26) can start it. Geometry matches
de-set / frame3 so fsck_msdos accepts it.

    make-image.py <out.img> <tap.elf> [--json]
"""
...
def main():
    ...
    if len(args) != 2:
        raise SystemExit("usage: make-image.py <out.img> <tap.elf>")
    out, elf = args

    blobs = {"TAP.ELF": open(elf, "rb").read()}
    if blobs["TAP.ELF"][:4] != b"\x7fELF":
        raise SystemExit("make-image: TAP.ELF is not ELF")
    ...
    add("TAP.ELF", 0x20, chains["TAP.ELF"][0], len(blobs["TAP.ELF"]))
    ...
    if want_json:
        ...
        print("make-image: %s — TAP.ELF %d"
              % (os.path.basename(out), len(blobs["TAP.ELF"])))
