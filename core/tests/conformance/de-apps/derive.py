#!/usr/bin/env python3
"""core/tests/conformance/de-apps/derive.py

Host-side picture for TAP.ELF: geometry, colours, hit/miss clicks,
the flip scancode, and the hidden `go` spelling. Nothing is read
back out of the kernel at derivation time.

    derive.py <tap.c> <wm.dart> <wmevent.dart> <kbdq.dart> <osframe.h>
...
def main():
    if len(sys.argv) != 6:
        die("usage: derive.py <tap.c> <wm.dart> <wmevent.dart> "
            "<kbdq.dart> <osframe.h>")
    tapc,     wmdart, evdart, kbdqdart, frameh = sys.argv[1:6]
    src = open(tapc).read()
    P = c_defs(tapc)
    W = dart_ints(wmdart)
    E = dart_ints(evdart)
    ...
    F = c_defs(frameh)
    chrome = os.path.join(os.path.dirname(wmdart), "wmchrome.dart")
    C = dart_ints(chrome)
    if "wmTitleH" not in C:
        die("wmchrome.dart does not define wmTitleH")
    title_h = C["wmTitleH"]
    apph = os.path.join(os.path.dirname(tapc), "osxui_app.h")
    A = c_defs(apph)
    if "OSXUI_CSD_H" not in A:
        die("osxui_app.h does not define OSXUI_CSD_H")
    if A["OSXUI_CSD_H"] != title_h:
        die("OSXUI_CSD_H %d != wmTitleH %d — CSD and chrome title drifted"
            % (A["OSXUI_CSD_H"], title_h))

    for k in ("WIN_W", "WIN_H", "SURF_X", "SURF_Y", "SURF_FILL",
              "CTL_X", "CTL_Y", "CTL_W", "CTL_H", "CTL_OFF", "CTL_ON",
              "FLIP_SCAN", "WIN_PAGES"):
        if k not in P:
            die("tap.c does not define %s" % k)
    ...
    for k in ("SYS_WMSURFACE", "SYS_KBDEVENT", "SYS_WMEVENT", "SYS_SPAWN"):
        if k not in F:
            die("osframe.h does not name %s" % k)

    if "oslibc.h" in src:
        die("tap.c includes oslibc.h — TAP numbers live in osframe.h")
    if re.search(r"^#define\s+SYS_", src, re.M):
        die("tap.c copies SYS_* by hand — include osframe.h")
    if "SYS_FDWAIT" in src or re.search(r"#define\s+SYS_\w+\s+11\b", src):
        die("tap.c names fdwait — 11 stays reserved")
    if re.search(r"guest OS", src, re.I):
        die("tap.c says guest OS")
    if re.search(r"Flutter", src, re.I):
        die("tap.c names Flutter")
    ...
    if P["WIN_PAGES"] != need_pages:
        die("tap.c asks for %d pages but %dx%d at 4 bytes per pixel needs %d"
    ...
    if F["SYS_WMSURFACE"] != 23 or F["SYS_KBDEVENT"] != 24:
        die("osframe.h wmsurface/kbdevent are not 23/24")
    if F["SYS_WMEVENT"] != 25 or F["SYS_SPAWN"] != 26:
        die("osframe.h wmevent/spawn are not 25/26")

    hit_rx = cx + cw // 2
    hit_ry = cy + ch // 2
    # ADR-0195: TAP paints CSD into the top wmTitleH rows. A miss at (8,8)
    # sits in the pearl band (and is a title-drag under wm de). The fill
    # probe and the miss click must be in the client body.
    miss_rx = 8
    miss_ry = title_h + 8
    if hit_ry < title_h:
        die("hit point y %d sits in the CSD title band (%d)" % (hit_ry, title_h))
    if miss_ry < title_h:
        die("miss point y %d sits in the CSD title band (%d)" % (miss_ry, title_h))
    ...
    print("syscall_spawn=%d" % F["SYS_SPAWN"])
    ...
    print("hit_line=TAP HIT %08X" % (on & 0xFFFFFF))
    print("ready_line=TAP READY")
    print("miss_line=TAP MISS")
    print("go_cmd=go tap.elf")
    print("go_line=GO")
    print("ghost_cmd=go ghost.elf")
    print("ghost_err=no such name in the root directory")
    ...
    print("control_idle=ctl_is_not_on %d %d %08X" % (ctl_px, ctl_py, on & 0xFFFFFF))
    print("control_miss=miss_is_not_on %d %d %08X" % (ctl_px, ctl_py, on & 0xFFFFFF))
    print("control_fill=fill_is_not_on %d %d %08X" % (fill_px, fill_py, on & 0xFFFFFF))
