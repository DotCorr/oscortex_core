...
ck; [[ "$EV_SIZE" -eq 1920 ]] || fail "wmeventStore is ${EV_SIZE:-missing} bytes, expected 1920 (one ring per wmMaxWindows)"
...
ck; [[ "$EXTERN_COUNT" -eq 44 ]] \
  || fail "kmain.o declares $EXTERN_COUNT externs, expected 44 — honored plat $HONORED_PLAT/$PLAT_PRESENT and msr_write are subtracted by name; D2 added no assembly"
