#!/usr/bin/env bash
# core/tests/conformance/de-graphite4/run.sh
#
# ADR-0159 — Desktop fill is Graphite drawRect.
# docs/design/c-modules.md, GAP-0313 leftover after ADR-0153.
#
# WHAT THIS ASSERTS
# ---------------------------------------------------------------------------
# Venus arms MakeVulkan. Former CPU desktop/taskbar fill is Graphite
# drawRect + ICD DRAW (not put_px). Serial OSGFX GRAPHITE DESK 001C6A38.
# Keeps RRECT / PIX. Homebrew stays NONE / no DESK. Curved MakeRectXY
# is named leftover (still GPs in Recorder::snap on this ICD).
#
# Usage: bash run.sh
...
fail() { echo "DE-graphite4: FAIL — $1" >&2; exit 1; }
setup_error() { echo "DE-graphite4: FAIL — $1" >&2; exit 2; }
...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-de-graphite4.XXXXXX")" \
...
ADR="$CORE_DIR/docs/decisions/0159-desktop-fill-is-graphite-drawrect.md"
...
echo "=== STRUCTURAL ==="
ck; [[ -f "$ADR" ]] || fail "ADR-0159 is missing"
ck; grep -q '0158 is two-mappers' "$ADR" || fail "ADR-0159 lost 0158 mention"
ck; grep -q 'MakeRectXY' "$ADR" || fail "ADR-0159 must name curved MakeRectXY leftover"
...
ck; grep -q 'graphite-desk-gpu' "$GRAPHITE_GUEST" \
  || fail "graphite-desk-gpu token missing"
ck; grep -q 'graphite-rrect-gpu' "$GRAPHITE_GUEST" \
  || fail "do not drop graphite-rrect-gpu"
ck; grep -q 'OSGFX GRAPHITE DESK' "$GRAPHITE_GUEST" \
  || fail "DESK serial line missing"
ck; grep -q '0x001C6A38' "$GRAPHITE_GUEST" \
  || fail "Graphite desktop fill colour missing"
ck; grep -q 'drawRect' "$GRAPHITE_GUEST" \
  || fail "Graphite desk path does not call drawRect"
ck; grep -q 'osgfx_graphite_fill_desk' "$GRAPHITE_GUEST" \
  || fail "osgfx_graphite_fill_desk missing"
ck; grep -q 'osgfx_graphite_fill_desk' "$SKIA_CPP" \
  || fail "osgfx_skia.cpp does not call Graphite fill_desk"
ck; awk '/OSGFX_CHROME \|\| rgb == \(uint32_t\)OSGFX_DESK/,/put_px/' "$SKIA_CPP" \
  | grep -q 'return;' \
  || fail "Graphite desk/taskbar path still falls through to CPU put_px"
...
ck; ! grep -q 'de-graphite4\|GRAPHITE DESK\|0159' \
      "$CORE_DIR/docs/syscall-registry.md" \
  || fail "ADR-0159 added a syscall"
echo "STRUCTURAL: pass  ADR-0159; desk drawRect; no CPU fallback"
...
echo "ANTI-VACUITY: pass  CPU-only has no Graphite desk/rrect token"
...
ck; elf_has "$KERNEL_ELF" "graphite-desk-gpu" \
  || fail "kernel.elf has no graphite-desk-gpu"
...
echo "GRAPHITE LINK: pass  desk + rrect tokens + MakeVulkan + ICD"
...
echo "HOMEBREW: pass  NONE path; no DESK; no RRECT; no PIX; WM GFX ON"
...
# Match de-graphite3: no fb on virtio-gl (FB NONE). DESK/RRECT proof is in
# graphite_try beside PIX — does not need sit-in scanout. Extra wait for
# the larger desktop fill.
GL_KEYS="$(typekeys 'virtgpuv'),ret,wait:8000"
GL_KEYS="$GL_KEYS,$(typekeys 'wm gfx'),ret,wait:25000"
...
echo "VENUS: pass  OK; PIX; RRECT; DESK 001C6A38; WM GFX ON"

require_assertions "$ASSERTIONS_REQUIRED"
echo "DE-graphite4: PASS — Graphite desktop fill (not CPU put_px); Homebrew NONE ($ASSERTIONS checks)"
exit 0
