/* core/tests/conformance/de-shm/c.c
 *
 * Third of three derived surfaces. The slot this would have been
 * wmRetNoSpace when shmMax was 2.
 */
...
static volatile u64 marker = 0x00C300C300C300C3UL;

static const char msg_line[] = "DE SHM C\n";
static const char msg_commit[] = "DE SHM C COMMIT\n";
...
void _start(void) {
  ...
  if (frames >= WM_FLOOR) {
    die(0x0C000104UL | (frames << 32));
  }
  wr(msg_commit, sizeof(msg_commit) - 1);
  if (marker != 0x00C300C300C300C3UL) {
    die(0x0C000106UL);
  }
