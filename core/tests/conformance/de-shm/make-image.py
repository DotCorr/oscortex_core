#!/usr/bin/env python3
"""core/tests/conformance/de-shm/make-image.py

FAT16 volume with three 8.3 ELFs: A.ELF, B.ELF, C.ELF.

    make-image.py <out.img> <a.elf> <b.elf> <c.elf>
"""
...
def main():
    if len(sys.argv) != 5:
        raise SystemExit("usage: make-image.py <out.img> <a.elf> <b.elf> <c.elf>")
    out, e1, e2, e3 = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]

    order = ("A.ELF", "B.ELF", "C.ELF")
    blobs = {
        "A.ELF": open(e1, "rb").read(),
        "B.ELF": open(e2, "rb").read(),
        "C.ELF": open(e3, "rb").read(),
    }
    if len(set(blobs.values())) != 3:
        raise SystemExit("make-image: two of the ELFs are byte-identical")
    ...
    print("make-image: %s — A.ELF %d, B.ELF %d, C.ELF %d"
          % (os.path.basename(out), len(blobs["A.ELF"]),
             len(blobs["B.ELF"]), len(blobs["C.ELF"])))
