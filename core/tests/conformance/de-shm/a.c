/* core/tests/conformance/de-shm/a.c
 *
 * First of three derived surfaces. Attach, paint, commit, yield.
 * Stays mapped so B.ELF and C.ELF can join it (ADR-0109).
 */
...
static volatile u64 marker = 0x00A100A100A100A1UL;

static const char msg_line[] = "DE SHM A\n";
static const char msg_commit[] = "DE SHM A COMMIT\n";
...
void _start(void) {
  ...
  if (frames >= WM_FLOOR) {
    die(0x0A000104UL | (frames << 32));
  }
  wr(msg_commit, sizeof(msg_commit) - 1);
  if (marker != 0x00A100A100A100A1UL) {
    die(0x0A000106UL);
  }
