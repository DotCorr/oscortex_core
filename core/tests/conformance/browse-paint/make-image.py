#!/usr/bin/env python3
"""core/tests/conformance/browse-paint/make-image.py

FAT16 volume with PAINT.ELF and NOPAIN.ELF so hidden `go` can start
each twin. Geometry matches de-apps so fsck_msdos accepts it.

    make-image.py <out.img> <paint.elf> <nopain.elf>
"""
import os
import struct
import sys
...
def main():
    if len(sys.argv) != 4:
        raise SystemExit("usage: make-image.py <out.img> <paint.elf> <nopain.elf>")
    out, elf1, elf2 = sys.argv[1], sys.argv[2], sys.argv[3]

    blobs = {
        "PAINT.ELF": open(elf1, "rb").read(),
        "NOPAIN.ELF": open(elf2, "rb").read(),
    }
    if blobs["PAINT.ELF"][:4] != b"\x7fELF":
        raise SystemExit("make-image: PAINT.ELF is not ELF")
    if blobs["NOPAIN.ELF"][:4] != b"\x7fELF":
        raise SystemExit("make-image: NOPAIN.ELF is not ELF")
    if blobs["PAINT.ELF"] == blobs["NOPAIN.ELF"]:
        raise SystemExit("make-image: the two ELFs are byte-identical")

    taken = set()
    chains = {}
    c = 2
    for name in ("PAINT.ELF", "NOPAIN.ELF"):
    ...
    add("PAINT.ELF", 0x20, chains["PAINT.ELF"][0], len(blobs["PAINT.ELF"]))
    add("NOPAIN.ELF", 0x20, chains["NOPAIN.ELF"][0], len(blobs["NOPAIN.ELF"]))
    ...
    print("make-image: %s — PAINT.ELF %d, NOPAIN.ELF %d"
          % (os.path.basename(out), len(blobs["PAINT.ELF"]),
             len(blobs["NOPAIN.ELF"])))
