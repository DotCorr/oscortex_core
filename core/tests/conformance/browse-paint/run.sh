#!/usr/bin/env bash
# core/tests/conformance/browse-paint/run.sh
#
# ADR-0115 / ADR-0122 / ADR-0123 — the running OS calls oschrome.h.
# PAINT.ELF is the thin FRAME client; oschrome_guest.c is the
# ABI + HTML rgb() painter; official linux64 libcef.so supplies
# cef_initialize (not Mac CEF, not a handwritten stub). A data:
# page paints PAGE; --no-init / NOPAIN.ELF is not PAGE. nm of
# PAINT.ELF (what QEMU runs) names cef_initialize. Stub-only
...
PROVE="$CORE_DIR/tests/conformance/de-browse/prove-onpaint-block.py"
ADR166="$CORE_DIR/docs/decisions/0166-onpaint-stand-in-delivers-pixels.md"
FETCH64="$CORE_DIR/scripts/fetch-cef-linux64.sh"
EXTRACT="$CORE_DIR/scripts/extract-cef-guest.sh"

fail() { echo "BROWSE-paint: FAIL — $1" >&2; exit 1; }
setup_error() { echo "BROWSE-paint: FAIL — $1" >&2; exit 2; }
...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-browse-paint.XXXXXX")" \
...
ck; [[ -f "$ADR166" ]] || setup_error "no ADR-0166"
...
ck; grep -q 'oschrome_on_paint' "$GUEST_C" \
  || fail "oschrome_guest.c has no oschrome_on_paint — stand-in missing"
ck; grep -q 'oschrome_on_paint' "$HDR" \
  || fail "oschrome.h has no oschrome_on_paint"
ck; grep -q 'fill_bgra' "$GUEST_C" \
  || fail "oschrome_guest.c has no fill_bgra — BGRA staging missing"
ck; grep -q 'OSCHROME_PET_VIEW' "$GUEST_C" \
  || fail "oschrome_guest.c has no OSCHROME_PET_VIEW"
ck; grep -q 'BGRA' "$GUEST_C" \
  || fail "oschrome_guest.c does not name BGRA — not the CEF OSR ABI"
ck; grep -q 'Leftover: wire official libcef.so' "$GUEST_C" \
  || fail "oschrome_guest.c lost the honest leftover label"
ck; grep -q 'not a rename of parse_rgb' "$GUEST_C" \
  || fail "oschrome_guest.c lost the anti-rename claim"
# Anti-rename: load_url must not set painted=1 before on_paint.
ck; ! awk '/int oschrome_load_url/,/^}/' "$GUEST_C" | grep -q 'painted = 1' \
  || fail "oschrome_load_url still sets painted=1 — that is the old rgb plant"
ck; awk '/int oschrome_load_url/,/^}/' "$GUEST_C" | grep -q 'pending = 1' \
  || fail "oschrome_load_url does not set pending — on_paint is never scheduled"
ck; grep -q 'BROWSE_NO_ONPAINT' "$BROWSE_C" \
  || fail "browse.c has no BROWSE_NO_ONPAINT twin"
...
ck; grep -q 'ASSERTIONS_REQUIRED=111' "$SCRIPT_DIR/run.sh" \
  || fail "browse-paint floor moved off 111"
ck; grep -q 'ASSERTIONS_REQUIRED=87' "$CORE_DIR/tests/conformance/de-browse/run.sh" \
  || fail "de-browse floor moved off 87 — do not raise it on this door"
ck; ! grep -qiE 'guest OS' "$BROWSE_C" "$GUEST_C" "$CEF_C" "$ADR" "$ADR122" "$ADR123" \
  || fail "BROWSE-paint sources say guest OS"
...
ck; ! grep -F -e 'PAINT.ELF' -e 'browse.c' -e 'BROWSE-paint' \
...
ck; ! grep -F -e 'PAINT.ELF' -e 'browse.c' \
...
ck; [[ -s "$WORKDIR/paint.elf" ]] || fail "no paint.elf"
ck; [[ -s "$WORKDIR/nopain.elf" ]] || fail "no nopain.elf"

NM=$(x86_64-elf-nm "$WORKDIR/paint.elf")
ck; echo "$NM" | grep -q 'oschrome_backend_chromium' \
  || fail "paint.elf has no oschrome_backend_chromium"
ck; echo "$NM" | grep -q 'oschrome_load_url' \
  || fail "paint.elf has no oschrome_load_url"
ck; echo "$NM" | grep -q 'oschrome_on_paint' \
  || fail "paint.elf has no oschrome_on_paint — nm of cef_initialize is not this"
ck; echo "$NM" | grep -E 'cef_initialize' >/dev/null \
  || fail "paint.elf has no cef_initialize — QEMU would not run official CEF"
ck; echo "$NM" | grep -E '[Tt] cef_initialize' >/dev/null \
  || fail "paint.elf cef_initialize is not a defined text symbol"
NMN=$(x86_64-elf-nm "$WORKDIR/nopain.elf")
ck; echo "$NMN" | grep -q 'oschrome_backend_chromium' \
  || fail "nopain.elf dropped oschrome_backend_chromium"
ck; echo "$NMN" | grep -E 'cef_initialize' >/dev/null \
  || fail "nopain.elf dropped cef_initialize"

echo
echo "=== ONPAINT STAND-IN (ADR-0166) ==="
ck; grep -q 'oschrome_on_paint' "$ADR166" \
  || fail "ADR-0166 does not name oschrome_on_paint"
ck; grep -q 'wire official libcef.so' "$ADR166" \
  || fail "ADR-0166 lost the honest leftover"
ck; grep -q 'not a rename' "$ADR166" \
  || fail "ADR-0166 lost the anti-rename clause"
# Official libcef OnPaint is still not executable — leftover holds.
...
echo "ONPAINT: stand-in in guest; official libcef OnPaint leftover; de-browse floor 87"

DISK_IMG="$WORKDIR/disk.img"
ck; python3 "$SCRIPT_DIR/make-image.py" "$DISK_IMG" \
  "$WORKDIR/paint.elf" "$WORKDIR/nopain.elf" \
...
if command -v hdiutil >/dev/null 2>&1; then
  ...
  ck; [[ -f "$MOUNTPOINT/PAINT.ELF" ]] || fail "mounted volume has no PAINT.ELF"
  ck; [[ -f "$MOUNTPOINT/NOPAIN.ELF" ]] || fail "mounted volume has no NOPAIN.ELF"
  hdiutil detach "$ATTACHED" >/dev/null 2>&1
  ATTACHED=""
  echo "IMAGE: pass  PAINT.ELF + NOPAIN.ELF on FAT"
else
...
cp "$WORKDIR/page/shot.png" "$CORE_DIR/build/browse-paint-page.png" 2>/dev/null || true

drive_boot "$WORKDIR/none" "$KEYS_NONE" "USER WRITE $NONE_LINE" "NONE"
SER_NONE="$WORKDIR/none/serial.txt"
FB_NONE="$WORKDIR/none/fb.bin"
cp "$WORKDIR/none/shot.png" "$CORE_DIR/build/browse-paint-none.png" 2>/dev/null || true
...
echo
echo "=== NOPAIN (anti-vacuity: callback off) ==="
...
if python3 "$PROBE" "$FB_NONE" "$PITCH_N" "$PROBE_X" "$PROBE_Y" "$PAGE_HEX" "none_must_fail"; then
  fail "NOPAIN probe ($PROBE_X,$PROBE_Y) is PAGE $PAGE_HEX — on_paint was not required"
fi
ck; true
echo "    nopain_body            ($PROBE_X,$PROBE_Y) is not PAGE — callback-off holds"

require_assertions "$ASSERTIONS_REQUIRED"
echo "BROWSE-paint: PASS — PAINT.ELF OnPaint stand-in delivers PAGE; --no-onpaint misses ($ASSERTIONS checks). Labelled stand-in; leftover: wire official libcef.so."
exit 0
