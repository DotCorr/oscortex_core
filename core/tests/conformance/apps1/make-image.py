#!/usr/bin/env python3
"""core/tests/conformance/apps1/make-image.py

FAT16 volume for the application system: two 8.3 FRAME ELFs and a
catalog APPS.TXT that lists them. Geometry matches frame1 / m15-fileio
so fsck_msdos accepts it.

    make-image.py <out.img> <app1.elf> <app2.elf> [--json]
"""
...
# Two names, one per line, no trailing blank. A catalog of one name, or
# an empty file, would let a harness that only greps one token pass.
CATALOG = b"APP1.ELF\nAPP2.ELF\n"
...
def main():
    ...
    if len(args) != 3:
        raise SystemExit("usage: make-image.py <out.img> <app1.elf> <app2.elf>")
    out, elf1, elf2 = args

    blobs = {
        "APP1.ELF": open(elf1, "rb").read(),
        "APP2.ELF": open(elf2, "rb").read(),
        "APPS.TXT": CATALOG,
    }
    if blobs["APP1.ELF"] == blobs["APP2.ELF"]:
        raise SystemExit("make-image: the two ELFs are byte-identical")
    if b"APP1.ELF" not in CATALOG or b"APP2.ELF" not in CATALOG:
        raise SystemExit("make-image: catalog does not list both apps")
    if CATALOG.count(b"\n") < 2:
        raise SystemExit("make-image: catalog must list two names")

    taken = set()
    chains = {}
    c = 2
    for name in ("APP1.ELF", "APP2.ELF", "APPS.TXT"):
    ...
    if want_json:
        ...
        print("make-image: %s — APP1.ELF %d, APP2.ELF %d, APPS.TXT %d"
              % (os.path.basename(out), len(blobs["APP1.ELF"]),
                 len(blobs["APP2.ELF"]), len(blobs["APPS.TXT"])))
