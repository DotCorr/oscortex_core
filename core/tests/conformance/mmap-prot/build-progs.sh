#!/usr/bin/env bash
# core/tests/conformance/mmap-prot/build-progs.sh
#
# Freestanding PROG for ADR-0163 (no oslibc — mprotect unnamed).
...
grep -q 'SYS_MPROTECT' "$SCRIPT_DIR/prog.c" || fail "prog.c never names SYS_MPROTECT"
grep -q 'SHM_FIXED' "$SCRIPT_DIR/prog.c" || fail "prog.c never names SHM_FIXED"
grep -q 'SHM_BADFIXED' "$SCRIPT_DIR/prog.c" || fail "prog.c never names SHM_BADFIXED"
