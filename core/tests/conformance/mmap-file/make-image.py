#!/usr/bin/env python3
"""core/tests/conformance/mmap-file/make-image.py

FAT16 volume with PROG.ELF and PLANT.DAT (two pages of known bytes).
...
def plant_bytes():
    """Two pages. Magics at page heads; page0[8]=0xA5; rest patterned."""
    b = bytearray(PAGE * 2)
    b[0:8] = b"MMAPFILE"
    b[8] = 0xA5
    for i in range(9, PAGE):
        b[i] = (0x40 + (i & 0x3F)) & 0xFF
    b[PAGE:PAGE + 8] = b"DEMANDPG"
    for i in range(8, PAGE):
        b[PAGE + i] = (0x80 + (i & 0x3F)) & 0xFF
    return bytes(b)
...
def main():
    ...
    plant = plant_bytes()
    need_elf = max(1, (len(blob) + CLUSTER_BYTES - 1) // CLUSTER_BYTES)
    need_plant = max(1, (len(plant) + CLUSTER_BYTES - 1) // CLUSTER_BYTES)
    chain_elf = list(range(2, 2 + need_elf))
    chain_plant = list(range(2 + need_elf, 2 + need_elf + need_plant))
    if chain_plant[-1] >= CLUSTER_COUNT + 2:
        raise SystemExit("make-image: volume too small")
    ...
    for chain in (chain_elf, chain_plant):
    ...
    root.extend(dir_entry(eightthree("PROG.ELF"), 0x20, chain_elf[0], len(blob)))
    root.extend(dir_entry(eightthree("PLANT.DAT"), 0x20, chain_plant[0], len(plant)))
    ...
    def plant_chain(chain, data):
        for i, cl in enumerate(chain):
            piece = data[i * CLUSTER_BYTES:(i + 1) * CLUSTER_BYTES]
            piece = piece + b"\0" * (CLUSTER_BYTES - len(piece))
            img[cluster_at(cl):cluster_at(cl) + CLUSTER_BYTES] = piece

    plant_chain(chain_elf, blob)
    plant_chain(chain_plant, plant)

    with open(out, "wb") as f:
        f.write(bytes(img))
    print("make-image: %s — PROG.ELF %d bytes, PLANT.DAT %d bytes"
          % (os.path.basename(out), len(blob), len(plant)))
