...
ck; [[ -f "$DRIVER" ]] || setup_error "QMP driver not found at $DRIVER (m5-pci reuses m2-console's)"
...
ck; WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/oscortex-m5.XXXXXX")" || setup_error "could not create a temp workdir"
...
ck; if [[ "$NON_PMM_BSS" -ne 440 ]]; then
  fail "the kernel holds $(( KDATA_BSS + ASM_BSS )) bytes of mutable static storage, of which $PMM_STORE_SIZE are M7's pmmStore and $VM_STORE_SIZE are M8's vmStore, leaving $NON_PMM_BSS — expected 440 (392 through M4, plus 48 for the framebuffer console's state — 32 at M5, plus the two geometry words ADR-0064's fallback chain added so a live VirtIO/GOP aperture can outrank the Bochs 800x600 default; PCI enumeration adds NONE). That number is the measured cost of DCDart having no mutable statics (known-gaps GAP-0053) — if you meant to change it, change it in kdata.S's header and in GAP-0053 too."
fi
FB_STATE_SIZE=$(bsssize fbStateBlock)
ck; [[ "$FB_STATE_SIZE" == "48" ]] || fail "fbStateBlock is ${FB_STATE_SIZE:-missing} bytes, expected 48 (six u64: base, pitch, cursor column, cursor row, and ADR-0064's geometry width/height) — fbSetState indexes it by fixed offset and a shorter block would write past the end of its own object"
echo "STRUCTURAL: pass  440 bytes of mutable static storage outside M7's page-allocator and M8's page-table blocks (392 through M4 + 48 for fbStateBlock; PCI enumeration added zero)"
...
check_table() {
  ...
  ck; [[ -n "$got" ]] || fail "$sym not found in kmain.o — a @rodata table M5 depends on was not emitted"
...
ck; [[ -n "$RODATA_OFF_HEX" ]] || fail "pciClassNames has no symbol value in kmain.o"
RODATA_OFF=$((16#$RODATA_OFF_HEX))
ck; x86_64-elf-objcopy -O binary --only-section=.rodata "$CORE_DIR/build/kmain.o" "$WORKDIR/rodata.bin" \
  || fail "could not extract .rodata from kmain.o"
ck; if ! python3 - "$WORKDIR/rodata.bin" "$RODATA_OFF" <<'PY'
import sys
...
ck; if ! python3 "$SCRIPT_DIR/check-font.py" "$WORKDIR/rodata.bin" "$FONT_OFF"; then
...
for sym in port_inl port_outl port_inw port_outw; do
  ck; grep -q "$sym" <<<"$VERIFY_OUT" || fail "$sym is not in kmain.o's extern manifest — the code that should be calling it is not"
done
...
drive_session() {
  ...
  local qemu_pid=$!
  local drive_status
  run_status drive_status -- python3 "$DRIVER" --port "$port" --serial "$ser" --wait-for 'M1 END\n' --png "$png" --screen-text "$outdir/screen.txt" --monitor-command 'info pci' "${FB_DUMP_ARGS[@]}" --monitor-capture "$outdir/info-pci.txt" --keys "$keys"
  local qemu_status
...
ck; if ! python3 - "$SERIAL_CAPTURE" "$INFO_PCI" <<'PY'
import re, sys
...
# ---------------------------------------------------------------------------
ck; grep -q "^FB BAR FD000000 MODE 0320x0258x20 OK$" "$FB_SERIAL" || {
...
ck; grep -qi "BAR0: 32 bit prefetchable memory at 0xfd000000" "$FB_INFO_PCI" || {
...
FONT_OFF_HEX=$(x86_64-elf-readelf -sW "$CORE_DIR/build/kmain.o" | awk '$8=="fbFont8x16" {print $2; exit}')
FONT_OFF=$((16#$FONT_OFF_HEX))
ck; if ! python3 "$SCRIPT_DIR/check-pixels.py" \
...
ck; grep -q "^PCI TOTAL 0006$" "$SCREEN_TEXT" || {
  cat -n "$SCREEN_TEXT" >&2
...
ck; [[ -s "$FB_SHOT_PNG" ]] || fail "no framebuffer screenshot was produced at $FB_SHOT_PNG"
ck; case "$(head -c 8 "$FB_SHOT_PNG" | od -An -tx1 | tr -d ' \n')" in
  89504e470d0a1a0a) ;;
...
ck; if cmp -s "$FB_SERIAL" "$EXPECTED_SERIAL"; then
...
ck; if ! python3 - "$WORKDIR/bridge/serial.txt" "$WORKDIR/bridge/info-pci.txt" <<'PY'
import re, sys
serial = open(sys.argv[1], "rb").read().decode("latin-1")
info = open(sys.argv[2], "r", encoding="utf-8", errors="replace").read()
fails = []
...
ck; if cmp -s "$WORKDIR/bridge/serial.txt" "$EXPECTED_SERIAL"; then
...
ck; grep -q "^PCI TOTAL 0006$" "$WORKDIR/negative/serial.txt" || \
