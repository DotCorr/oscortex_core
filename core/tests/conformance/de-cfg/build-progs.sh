#!/usr/bin/env bash
# core/tests/conformance/de-cfg/build-progs.sh
#
# WIN.ELF — resident window that drains wmevent configure/enter/leave.
...
grep -q 'SYS_WMEVENT' "$SCRIPT_DIR/win.c" \
  || fail "win.c does not pop wmevent"
grep -q 'DE CFG COMMIT' "$SCRIPT_DIR/win.c" \
  || fail "win.c does not contain DE CFG COMMIT"
grep -q 'WMEVENT_TYPE_CONFIGURE' "$SCRIPT_DIR/win.c" \
  || fail "win.c does not name configure"
