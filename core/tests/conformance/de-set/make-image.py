#!/usr/bin/env python3
"""core/tests/conformance/de-set/make-image.py

FAT16 volume for Settings: SET.ELF and a planted FACTS.DAT the host
derived. Geometry matches frame3 / studio1 so fsck_msdos accepts it.

    make-image.py <out.img> <set.elf> <facts.bin> [--json]
"""
...
def main():
    ...
    if len(args) != 3:
        raise SystemExit("usage: make-image.py <out.img> <set.elf> <facts.bin>")
    out, elf, facts_path = args

    blobs = {
        "SET.ELF": open(elf, "rb").read(),
        "FACTS.DAT": open(facts_path, "rb").read(),
    }
    if len(blobs["FACTS.DAT"]) < 1:
        raise SystemExit("make-image: FACTS.DAT is empty")
    if blobs["SET.ELF"][:4] != b"\x7fELF":
        raise SystemExit("make-image: SET.ELF is not ELF")

    taken = set()
    chains = {}
    c = 2
    for name in ("SET.ELF", "FACTS.DAT"):
    ...
    add("SET.ELF", 0x20, chains["SET.ELF"][0], len(blobs["SET.ELF"]))
    add("FACTS.DAT", 0x20, chains["FACTS.DAT"][0], len(blobs["FACTS.DAT"]))
    ...
    layout = {
        "files": {n: {"bytes": len(blobs[n]), "clusters": len(chains[n])}
                  for n in blobs},
        "facts_bytes": len(blobs["FACTS.DAT"]),
    }
    if want_json:
        print(json.dumps(layout))
    else:
        print("make-image: %s — SET.ELF %d, FACTS.DAT %d"
              % (os.path.basename(out), len(blobs["SET.ELF"]),
                 len(blobs["FACTS.DAT"])))
