#!/usr/bin/env python3
"""core/tests/conformance/d7-click/d7-drive.py

Drives QEMU over QMP for D7. Same input vocabulary as qmp-drive.py
(rel:, btn:, wait:, qcodes) plus `until:<substring>` — wait until that
byte string appears in the serial capture. Composition is slow; a click
injected before both surfaces exist would be a desktop click and the
harness would fail for the wrong reason.
...
def die(msg):
    print(f"d7-drive: {msg}", file=sys.stderr)
    sys.exit(FAIL)
...
def main():
    ...
    print(f"d7-drive: connected, QEMU {qmp.version['major']}."
    ...
    print(f"d7-drive: kernel reached {args.wait_for!r}")
    time.sleep(0.5)
    ...
    for qcode in keys:
        ...
        time.sleep(0.05)
        typed += 1
    print(f"d7-drive: injected {typed} keystroke(s) and {pointed} pointer event(s)")
    ...
    print(f"d7-drive: screenshot {args.png}")
