/* core/tests/conformance/plat-need2/prog.c
 *
 * Named PLAT.ELF with PT_DYNAMIC and four DT_NEEDED (LIBC.SO,
 * LIBM.SO, LIBDL.SO, LIBPT.SO). Walks DT_NEEDED / DT_STRTAB,
 * dlopens each FAT name, and calls through the mapped face.
 * LINE1..LINE4 are MARK ^ MIX from write / need_fn / dl_fn /
 * pt_fn. ASK.ELF is the same bytes and is REFUSED 11. Missing
 * LIBPT.SO cannot invent LINE4. ADR-0160. Satisfies 4 of 32
 ...
 * SELF_BASE phdrs would read garbage. ADR-0160. */
...
const char msgStart[] = "NEED2 START";
 ...
 * inventing LINE4 without the fourth FAT file must fail. */
...
void progMain(void) {
  ...
  bad = loadNeeded();
  say("BAD", bad);
  sys(SYS_EXIT, 0xA1600000UL + bad, 0);
  for (;;) {
