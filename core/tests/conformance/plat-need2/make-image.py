#!/usr/bin/env python3
"""FAT16 volume for plat-need2 (ADR-0160).

Full: PLAT.ELF + ASK.ELF + LIBC.SO + LIBM.SO + LIBDL.SO + LIBPT.SO.
Partial: same ELFs + LIBC/LIBM/LIBDL (LIBPT.SO absent — no LINE4).
"""
...
def read_blob(path, label):
    data = open(path, "rb").read()
    if len(data) < 64 or len(data) > 65536:
        raise SystemExit("make-image: %s size out of range" % label)
    return data


def build(out, elf_path, so_paths):
    blob = read_blob(elf_path, "ELF")
    blobs = {"PLAT.ELF": blob, "ASK.ELF": blob}
    names = ["PLAT.ELF", "ASK.ELF"]
    for path in so_paths:
        base = os.path.basename(path).upper()
        if not base.endswith(".SO"):
            raise SystemExit("make-image: %r is not a .SO" % path)
        # Map build names to FAT 8.3 sonames.
        stem = base[:-3]
        if stem == "LIBC":
            fat = "LIBC.SO"
        elif stem == "LIBM":
    ...
    print("make-image: %s — PLAT/ASK %d, %s"
          % (os.path.basename(out), len(blob),
             ", ".join("%s %d" % (n, len(blobs[n])) for n in so_names)))


def main():
    if len(sys.argv) < 4:
        raise SystemExit(
            "usage: make-image.py <out.img> <plat.elf> <so...> "
            "(at least LIBC.SO)")
