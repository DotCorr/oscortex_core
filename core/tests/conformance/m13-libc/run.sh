...
ck; [[ "$KDATA_BSS" -eq 26240 ]] || fail "the kernel's mutable static storage outside M14's fatStore is $KDATA_BSS bytes, expected 18048 — M11/M12's 9664 plus M18's 64-byte scheduler header (ADR-0022), and not one byte of M13's. A C LIBRARY IS USERLAND. If the kernel needed new mutable state to host one, that is a different milestone and it needs its own ADR. This number carries the 4224 bytes the blocks BELOW it gained and no milestone here declared: ADR-0155 doubled pmmMaxFrames to 65536 so pmmStore went 4672 -> 8768, ADR-0189's larger fine map took vmStore 128 -> 240, and ADR-0064's scanout fallback chain put two geometry words in fbStateBlock, 32 -> 48."
# NO KERNEL *CODE* REACHES THE LIBRARY. Comments are stripped first, and that
# is not a loophole: the five places the kernel names oslibc.h today are doc
# comments on syscalls that say "No `oslibc.h` name" -- prose ASSERTING this
# very invariant, which the old raw `grep -r` then failed on. A check that
# fires on a sentence describing the rule cannot distinguish it from a
# violation of the rule, so it is asked of code. What replaces the lost reach
# is stricter, not looser: no kernel source may name the header at all in
# code, and none may import or include it under any spelling.
capture_sh LIBC_REACH LIBC_REACH_STATUS -- "python3 - '$CORE_DIR/kernel' <<'PY'
...
for f in sorted(pathlib.Path(sys.argv[1]).rglob('*.dart')):
    ...
    for n, line in enumerate(src.split('\n'), 1):
        ...
        if re.search(r'(import|include|part)\\b.*oslibc', line):
            bad.append('%s:%d: names the header in a directive: %s'
                       % (f.name, n, line.strip()))
if bad:
    raise SystemExit('\n'.join(bad))
print('    (no kernel source names oslibc outside a doc comment)')
PY"
echo "$LIBC_REACH"
ck; [[ $LIBC_REACH_STATUS -eq 0 ]] || fail "kernel code names oslibc — the C library must not be reachable from ring 0: $LIBC_REACH"
echo "STRUCTURAL: pass  kdata.o donates 13952 bytes of .bss outside M14's fat_store and no kernel CODE names the library — M13 is entirely userland"
