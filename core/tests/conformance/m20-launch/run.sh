...
#
# Mechanical check of M20's exit criterion (ADR-0027): ONE LAUNCH PATH, AND A
...
# needs. ADR-0027 deletes the M10 launch and routes `run` through `procCreate`,
...
#     three terminators) rather than a bare stack pointer. GAP-0153.
...
# ring 3; after ADR-0027 the only file that may call it is proc.dart.
# ---------------------------------------------------------------------------
ELF_ENTER=$(grep -c '^\s*enter_user(' "$CORE_DIR/kernel/elf.dart" || true)
[[ "$ELF_ENTER" -eq 0 ]] || fail "elf.dart calls enter_user() $ELF_ENTER time(s), expected 0 — the M10 launch path is back, and a program launched through it has no process slot and therefore no heap (ADR-0027)"
