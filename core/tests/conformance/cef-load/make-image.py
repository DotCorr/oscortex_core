#!/usr/bin/env python3
"""FAT16 volume: identical PLAT.ELF / ASK.ELF plus measured CEF.SO.

Only the 8.3 name may honour dlopen (ADR-0167). CEF.SO is the
official libcef measured slice — not a rename of oschrome_on_paint.
MISS.SO is deliberately absent.
"""
...
def main():
    if len(sys.argv) != 5:
        raise SystemExit("usage: make-image.py <out.img> <plat.elf> <cef.so> "
                         "<libc.so>")
    out, elf_path, so_path, libc_path = sys.argv[1:5]
    blob = open(elf_path, "rb").read()
    so = open(so_path, "rb").read()
    libc = open(libc_path, "rb").read()
    if len(blob) < 64 or len(so) < 64 or len(libc) < 64:
        raise SystemExit("make-image: ELF/.so is too small")
    if len(blob) > 65536 or len(so) > 65536 or len(libc) > 65536:
        raise SystemExit("make-image: image exceeds the 64 KiB cap")

    blobs = {"PLAT.ELF": blob, "ASK.ELF": blob, "CEF.SO": so,
             "LIBC.SO": libc}
    ...
    chains = {}
    c = 2
    for name in ("PLAT.ELF", "ASK.ELF", "CEF.SO", "LIBC.SO"):
    ...
    print("make-image: %s — PLAT/ASK %d bytes, CEF.SO %d, LIBC.SO %d "
          "(MISS.SO absent)"
          % (os.path.basename(out), len(blob), len(so), len(libc)))
