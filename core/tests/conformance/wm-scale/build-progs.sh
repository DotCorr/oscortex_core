#!/usr/bin/env bash
# core/tests/conformance/wm-scale/build-progs.sh
set -uo pipefail
...
x86_64-elf-ld -T "$SCRIPT_DIR/prog.ld" -z max-page-size=0x1000 --build-id=none -o "$OUT/prog.elf" "$OUT/prog.o" || fail "ld"
