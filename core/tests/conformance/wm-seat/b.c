/* core/tests/conformance/wm-seat/b.c — seat 1 while shell still has keys. */
...
static const char msg[] = "WM SEAT B\n";
static char line[48];
void _start(void) {
  wr(msg, sizeof(msg)-1);
  u64 h = sys1(SYS_SHMCREATE, 8);
  if (h >= WM_FLOOR) { sys1(SYS_EXIT, 1); for(;;){} }
  desc[0]=WM_ATTACH; desc[1]=h; desc[2]=200; desc[3]=80; desc[4]=64; desc[5]=64; desc[6]=0; desc[7]=0;
  ...
  desc[0]=WM_SEAT; desc[1]=h; desc[2]=1;
  sys1(SYS_WMSURFACE, (u64)desc);
  desc[0]=WM_SEATGET;
  u64 bits = sys1(SYS_WMSURFACE, (u64)desc);
  unsigned at=0; const char *p="WM SEAT B BITS ";
  while (*p) line[at++]=*p++;
