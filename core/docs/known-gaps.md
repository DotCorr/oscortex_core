...
## GAP-0160 — There is no blocking receive; an IPC receiver polls
...
## GAP-0161 — A message cannot convey authority
...
## GAP-0162 — Ports are integers agreed by convention, not names
...
## GAP-0163 — A channel has exactly two endpoints; there is no multicast
...
which would have to walk N endpoints — which is GAP-0160 again, from the other side.

---

## GAP-0164 — IPC requires a process; an M9 payload and an M10 `run` program cannot use it
...
## GAP-0165 — The channel's correctness depends on one core and no fences
...
## GAP-0166 — `chanRetCorrupt` is unreachable from ring 3, so no boot exercises it
...
produce. (The other is `chanRetBusy` — GAP-0167.)
...
## GAP-0167 — `chanRetBusy` is reachable but unexercised: `proc coop` starts exactly two processes
...
already taken, or one that is half-closed. Unlike `chanRetCorrupt` (GAP-0166) this is a perfectly
...
for other reasons — the port number arriving in `argv` (GAP-0162 and GAP-0149 together), at which
