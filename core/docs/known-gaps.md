...
## GAP-0152 — RESOLVED: a program could not have both argv and a heap

**Domain:** kernel (M10/M11/M19/M20)
**Status:** **RESOLVED** by `docs/decisions/0027-one-launch-path.md`, verified by
...
## GAP-0153 — `proc run` gets a complete stack but an empty argv

**Domain:** kernel (M20)
**Status:** OPEN — a bounded consequence of ADR-0027, recorded so nobody reads M20 as bigger than it is.

Since ADR-0027 every process gets a System V initial stack built by `argsBuild`. `proc run` names its
...
## GAP-0154 — `run` now refuses on a machine without FXSR

**Domain:** kernel (M20)
**Status:** OPEN — a deliberate behavioural narrowing, stated so it is not discovered as a surprise.

Before ADR-0027 an M10 `run` program had no process slot, no FPU save area and no need of one, so it
...
## GAP-0155 — A second `paging_install` bracket in the shell launcher loses control after the last exit

**Domain:** kernel (M20)
**Status:** OPEN — **worked around, not solved.** The cost is that one diagnostic had to move, and
that nobody yet knows why it had to.

While implementing ADR-0027 the page report (`elfPageReport`/`elfWindowLine`, which walk from CR3)
...
## GAP-0156b — m12-heap's flake is RFLAGS bit 16 (RF), and it is in the golden
...
the same tree earlier the same session. Nothing in ADR-0027 touches RFLAGS.
